...
int RAND_DRBG_generate(
  ...
  if (commentsOnError != 0) {
    *commentsOnError << "DEBUG: got to drbg generate!!\n";
  }
  int reseed_required = 0;
  if (drbg->state != DRBG_READY) {
    /* try to recover from previous errors */
    if (commentsOnError != 0) {
      *commentsOnError << "DEBUG: try to recover from previous errors by restarting. ";
    }
    rand_drbg_restart(drbg, NULL, 0, 0, commentsOnError);
    if (commentsOnError != 0) {
      *commentsOnError << "DEBUG: restart FINISHED.\n";
    }
    if (drbg->state == DRBG_ERROR) {
  ...
  if (drbg->parent != NULL) {
    ...
    if (reseed_counter > 0 && tsan_load(&drbg->parent->reseed_prop_counter) != reseed_counter) {
      reseed_required = 1;
    }
  }
