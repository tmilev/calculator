...
void AnotherWeylGroup<scalar, templateVector>::computeRho() {
  if (this->unrationalCartanSymmetric.numberOfRows < cartanSymmetric.numberOfRows) {
    int den = 1;
    ...
    for (int i = 0; i < this->cartanSymmetric.numberOfRows; i ++) {
      for (int j = 0; j < this->cartanSymmetric.numberOfColumns; j ++) {
        this->unrationalCartanSymmetric.elements[i][j] = (cartanSymmetric.elements[i][j] * den).getNumerator().getIntValueTruncated();
...
void computeTauSignatures(WeylGroupData* G, List<List<bool> >& tauSignatures, bool pseudo = false) {
  ...
  for (int i = 0; i < numCycles - 2; i ++) {
    ...
    for (int j = 0; j < sel.cardinalitySelection; j ++) {
      generators[j].makeSimpleReflection(sel.elements[j], *G);
    }
    getTauSignaturesFromSubgroup(*G, generators, tss[i]);
  }
  Vector<Rational> Xs;
  ...
  if (pseudo) {
    ...
    for (int i = 0; i < numCycles - 1; i ++) {
      ...
      List<bool> ts;
      getTauSignaturesFromSubgroup(*G, generators, ts);
      tss.addOnTop(ts);
      sel.incrementSelection();
    }
  }
