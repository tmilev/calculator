...
void LaTeXcrawler::BuildFreecalC() {
  ...
  for (int i = 0; i < numLecturesToProcess; i ++) {
    ...
    theReport.Report(reportStream.str());
    global.CallSystemNoOutput(currentSysCommand, &global);
    reportStream << "<b>[x2]</b>";
    theReport.Report(reportStream.str());
    global.CallSystemNoOutput(currentSysCommand, &global);
...
bool LaTeXcrawler::ExtractPresentationFileNames(std::stringstream* commentsOnFailure, std::stringstream* commentsGeneral) {
  ...
  for (int i = 0; i < this->slideFileNamesVirtualWithPatH.size; i ++) {
    if (StringRoutines::StringBeginsWith(this->slideFileNamesVirtualWithPatH[i], "LaTeX: ", &this->latexSnippets[i])) {
      if (i == 0) {
        if (commentsOnFailure != nullptr) {
    ...
    if (!StringRoutines::StringEndsWith(this->slideFileNamesVirtualWithPatH[i], ".tex")) {
