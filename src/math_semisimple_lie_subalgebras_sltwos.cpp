...
void SlTwoSubalgebra::initializeContainerSubalgebras(
  ...
  roots = currentMinimalRegularContainer.simpleRootsReductiveSubalgebra;
  ...
  this->indicesContainingRootSubalgebras.addOnTop(
    indexCurrentMinimalRegularContainer
  );
...
void SlTwoSubalgebra::fromSlTwoSubalgebraCandidate(
  ...
  this->hCharacteristic = input.hCharacteristic;
...
std::string SlTwoSubalgebras::toStringSummary(FormatExpressions* format) {
  ...
  << "e(P, P_0)= 0. ";
  if (this->unsuitableHCharacteristics.size > 0) {
    bool allbadAreGoodInTheirBadness = true;
    for (int i = 0; i < this->unsuitableHCharacteristics.size; i ++) {
      bool isGoodInItsbadness = false;
      for (const SlTwoSubalgebra& current : this->allSubalgebras) {
        if (
          this->unsuitableHCharacteristics[i] ==
          current.hElement.getCartanPart()
        ) {
