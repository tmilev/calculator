...
size_t rand_pool_acquire_entropy(RAND_POOL *pool, std::stringstream* commentsOnError) {
  if (commentsOnError != 0) {
    *commentsOnError << "DEBUG: acquiring entropy ...";
  }
  size_t bytes_needed = rand_pool_bytes_needed(pool, 1 /*entropy_factor*/);
  unsigned char *buffer = 0;
  if (commentsOnError != 0) {
    *commentsOnError << "DEBUG: getting  " << bytes_needed << " entropy bytes...\n";
  }
  ssize_t bytes;
  int randomDataFileDescriptor = open("/dev/urandom", O_RDONLY);
  if (randomDataFileDescriptor < 0) {
    if (commentsOnError != 0) {
      *commentsOnError << "Failed to open dev/urandom.\n";
    }
    return 0;
  }
  ...
  while (bytes_needed != 0 && attempts -- > 0) {
    ...
    bytes = read(randomDataFileDescriptor, buffer, bytes_needed);
    if (bytes > 0) {
