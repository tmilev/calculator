...
int tls_construct_client_hello(SSL *s, WPACKET *pkt)
{
    ...
    if (SSL_IS_DTLS(s)) {
        ...
        i = (s->hello_retry_request == sslData::SSL_HRR_NONE);
    }
    ...
    if (s->new_session || s->session->ssl_version == TLS1_3_VERSION) {
        if (s->version == TLS1_3_VERSION
            ...
            if (s->hello_retry_request == sslData::SSL_HRR_NONE
                    && RAND_bytes(s->tmp_session_id, sess_id_len, 0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR,
...
static int tls_construct_cke_rsa(SSL *s, WPACKET *pkt)
{
    ...
    if (RAND_bytes(pms + 2, (int)(pmslen - 2), 0) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_CKE_RSA, ERR_R_MALLOC_FAILURE);
        goto err;
    }
