...
bool GlobalVariables::Test::builds() {
  ...
  if (FileOperations::fileExistsVirtual("calculator")) {
    global.fatal << "Command make clean did not remove the calculator executable. " << global.fatal;
  }
  ...
  if (result != 0) {
    global << Logger::red
    << "Calculator most restrictive build generated errors. " << Logger::endL;
  }
  std::stringstream commentsOnFailure;
  if (!FileOperations::fileExistsVirtual("calculator", true, true, &commentsOnFailure)) {
    global.fatal << "Failed to build the calculator executable. "
