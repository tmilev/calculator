...
std::string SyntacticElement::toStringSyntaxRole(
  const CalculatorParser& owner
) const {
  ...
  *(*this->currentSyntacticStack).lastObject();
  ...
  for (
    ...
    if (currentElt.controlIndex != this->conExpression()) {
      out
      << " <b style='color:red'>"
      << currentElt.toStringSyntaxRole(*this)
      << "</b> ";
    } else {
...
bool CalculatorParser::replaceOXEXEXEXByE() {
  SyntacticElement& opElt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    8
  ];
  ...
  newExpr.addChildAtomOnTop(
    this->getOperationIndexFromControlIndex(opElt.controlIndex)
  );
...
bool CalculatorParser::replaceXXXByConCon(
...
bool CalculatorParser::replaceXByO(int operation) {
  ...
  this->conExpression();
  (*this->currentSyntacticStack)[(*this->currentSyntacticStack).size - 1
  ].data.makeAtom(*this->owner, operation);
  return true;
}
...
bool CalculatorParser::isInterpretedAsEmptySpace(const std::string& input) {
  ...
  return
  this->isInterpretedAsEmptySpace(
...
bool CalculatorParser::replaceXEXByEContainingOE(int inputOpIndex) {
  SyntacticElement& outputElt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    3
  ];
  SyntacticElement& inputElt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
...
bool CalculatorParser::replaceXdotsXByMatrixStart(int numberOfXs) {
  SyntacticElement& currentElt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    numberOfXs
  ];
...
bool CalculatorParser::replaceMatrixEXByMatrixNewRow() {
  SyntacticElement& matrixElt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    3
  ];
...
std::string CalculatorParser::toStringIsCorrectAsciiCalculatorString(
  ...
  if (badCharacters.size > 0) {
    ...
    for (int i = 0; i < badCharacters.size; i ++) {
      hexCodes.addOnTop(
        StringRoutines::convertByteToHex(badCharacters[i])
      );
    }
...
bool CalculatorParser::replaceSsSsXdotsXbySsXdotsX(int numberOfDots) {
  ...
  left.data.setExpectedSize(
    left.data.size() + right.data.size() - 1
  );
...
bool CalculatorParser::replaceEPowerMinusEXByEX() {
  ...
  Expression exponent;
  exponent.makeOX(
    *this->owner, this->owner->opMinus(), minusExponent.data
  );
  Expression incoming;
  incoming.makeXOX(
    *this->owner, this->owner->opPower(), base.data, exponent
  );
  base.data = incoming;
  return this->decreaseStackExceptLast(3);
}
...
bool CalculatorParser::isSeparatorFromTheRightForList(
  const std::string& input
) {
  return
  input == "}" ||
...
bool CalculatorParser::extractExpressions(
  ...
  for (
    this->counterInSyntacticSoup = 0; this->counterInSyntacticSoup < (
      *this->currrentSyntacticSoup
    ).size; this->counterInSyntacticSoup ++
  ) {
    counterReport ++;
    if (counterReport >= symbolsToIssueReport) {
      ...
      if (
        ...
        << " out of "
        << (*this->currrentSyntacticSoup).size
        << " syntactic elements. ";
        report.report(reportStream.str());
      }
    }
    (*this->currentSyntacticStack).addOnTop((*this->currrentSyntacticSoup)[
        this->counterInSyntacticSoup
      ]
    );
    if (this->flagLogSyntaxRules) {
...
bool CalculatorParser::applyOneRule() {
  ...
  this->controlSequences[secondToLastE.controlIndex];
  ...
  const SyntacticElement& sixthToLastE = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    6
  ];
  const std::string& sixthToLastS =
  this->controlSequences[sixthToLastE.controlIndex];
  ...
  if (this->owner->flagUseBracketsForIntervals) {
    if (thirdToLastS == "(" && secondToLastS == "Expression" && lastS == "]") {
      return
      this->replaceXEXByEContainingOE(
        this->owner->opIntervalRightClosed()
      );
    }
    if (thirdToLastS == "[" && secondToLastS == "Expression" && lastS == ")") {
      return
      this->replaceXEXByEContainingOE(
        this->owner->opIntervalLeftClosed()
      );
    }
    if (thirdToLastS == "[" && secondToLastS == "Expression" && lastS == "]") {
      return
      this->replaceXEXByEContainingOE(this->owner->opIntervalClosed());
    }
    if (this->owner->flagUseBracketsForIntervals) {
  ...
  if (lastS == "ln" || lastS == "log" || lastS == "\\ln") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\log"));
  }
  ...
  if (lastS == "arcsin") {
    return
    this->replaceXByCon(
      this->controlSequences.getIndexNoFail("\\arcsin")
    );
  }
  if (lastS == "arccos") {
    return
    this->replaceXByCon(
      this->controlSequences.getIndexNoFail("\\arccos")
    );
  }
  if (lastS == "arctan") {
    return
    this->replaceXByCon(
      this->controlSequences.getIndexNoFail("\\arctan")
    );
  }
  if (lastS == "sin") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\sin"));
  }
  if (lastS == "cos") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\cos"));
  }
  if (lastS == "tan") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\tan"));
  }
  if (lastS == "cot") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\cot"));
  }
  if (lastS == "csc") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\csc"));
  }
  if (lastS == "sec") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\sec"));
  }
  if (lastS == "sqrt") {
    return
    this->replaceXByCon(this->controlSequences.getIndexNoFail("\\sqrt"));
  }
  ...
  if (lastS == "\\" && secondToLastS == "\\") {
    return
    this->replaceXXByCon(this->controlSequences.getIndexNoFail("\\\\"));
  }
