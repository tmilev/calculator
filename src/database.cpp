...
std::string DatabaseData::GetDatA()const
{ return "'" + this->GetDataNoQuotes() + "'";
}

std::string DatabaseData::GetDataNoQuotes()const
{ return HtmlRoutines::ConvertStringToURLString(this->value, false);
}

std::string DatabaseData::GetIdentifierNoQuotes()const
...
std::string DatabaseData::GetIdentifieR()const
...
//  stOutput << "Creating column pt 2.";
  DatabaseData columnName = columnNameUnsafe;
  DatabaseData tableName = tableNameUnsafe;
...
std::string DatabaseRoutines::ToStringTablE(const DatabaseData& inputTable)
...
{ MacroRegisterFunctionWithName("UserCalculator::FetchOneUserRowPartOne");
  ...
  if (theQuery.allQueryResultStrings.size == 0)
  { if (failureStream != 0)
    { if (this->username.value == "" && this->email.value != "")
        *failureStream << "Could not find entries for user: " << HtmlRoutines::ConvertStringToHtmlString(this->email.value, false) << ".";
      else
        *failureStream << "Could not find entries for user: " << HtmlRoutines::ConvertStringToHtmlString(this->username.value, false) << ".";
    }
    return false;
  }
...
{ MacroRegisterFunctionWithName("UserCalculator::DeleteMe");
  ...
  std::stringstream queryStream;
  DatabaseData deletedUserName = "deleted" + currentTime.theTimeStringNonReadable + this->username.value;
...
{ MacroRegisterFunctionWithName("UserCalculator::ComputeAndStoreActivationStats");
  ...
  (this->actualActivationToken.value, theGlobalVariables.hostWithPort,
...
{ MacroRegisterFunctionWithName("UserCalculator::IamPresentInTable");
  ...
  DatabaseData theTable = tableNameUnsafe;
  std::stringstream theQueryStream;
...
{ MacroRegisterFunctionWithName("DatabaseRoutines::InsertRow");
  DatabaseData primaryKey   = primaryKeyUnsafe;
  DatabaseData primaryValue = primaryValueUnsafe;
  DatabaseData tableName    = tableNameUnsafe;
  std::stringstream queryStream;
