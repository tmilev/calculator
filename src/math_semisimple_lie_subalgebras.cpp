...
"Let \\(h\\) be in the Cartan subalgebra. "
...
template < >
const List<DynkinSimpleType>::Comparator* FormatExpressions::getMonomialOrder<
  DynkinSimpleType
>() const {
  return nullptr;
}
...
std::string SemisimpleSubalgebras::toStringSubalgebraNumberWithAmbientLink(
  ...
  out
  << "Subalgebra "
  << candidate.weylNonEmbedded->dynkinType.toMathMLFinal(nullptr)
  << " &#8618; "
  << "<a href=\"./"
  << this->displayNameMainFile1NoPath
  << "\">"
  << this->owner->weylGroup.dynkinType.toMathMLFinal(format)
  << "</a>";
...
void SemisimpleSubalgebras::writeReportToFiles() {
  ...
  if (
    !FileOperations::writeFileVirtual(
      this->virtualNameMainFile1, this->toHTML(), &out
    )
  ) {
    global.comments << out.str();
  }
  FormatExpressions subalgebraFormat;
  subalgebraFormat.flagCandidateSubalgebraShortReportOnly = false;
  for (int i = 0; i < this->subalgebras.values.size; i ++) {
    this->writeSubalgebraToFile(&subalgebraFormat, i);
  }
}

std::string SemisimpleSubalgebras::toHTML() {
  FormatExpressions format;
  std::stringstream out;
  out
  ...
  this->currentFormat.flagCandidateSubalgebraShortReportOnly = true;
...
std::string SemisimpleSubalgebras::toStringSemisimpleSubalgebrasSummaryLaTeX(
  ...
  int numberNoLInfiniteRelationsFound = 0;
  int totalBadParabolics = 0;
  for (RealizedSemisimpleSubalgebra& realized : this->subalgebras.values) {
    ...
    numberNoLInfiniteRelationsFound +=
    candidate.numberOfCasesNoLInfiniteRelationFound;
    totalBadParabolics += candidate.numberOfBadParabolics;
  }
  if (totalBadParabolics > 0) {
  ...
  if (numberNoLInfiniteRelationsFound == 0) {
    out
    << "<br>In all "
    << numberOfFailingConeCondition
...
std::string SemisimpleSubalgebras::toString(
  ...
  out << this->toHTMLTableSubalgebraLinksTable(format) << "<hr>";
  if (this->flagRealForms) {
...
void SemisimpleSubalgebras::writeSubalgebraToFile(
  ...
  out << ".</div>";
  out << "<br>" << currentSubalgebra.toString(format);
...
std::string SemisimpleSubalgebras::toHTMLTableSubalgebraLinksTable(
  FormatExpressions* format
) {
  (void) format;
  std::stringstream out;
  ...
  for (int i = 0; i < this->subalgebras.size(); i ++) {
    ...
    << displayIndex
    << "'>"
    << subalgebra.weylNonEmbedded->dynkinType.toMathMLFinal(nullptr)
    << "</a></td>";
...
void SemisimpleSubalgebras::writeHCandidates(
  ...
  int baseRank = targetType.getRank() - 1;
  DynkinSimpleType smallType = targetType.getSmallestSimpleType();
  int indexNewRoot = *currentRootInjection.lastObject();
  int indexNewRootInSmallType =
  indexNewRoot - targetType.getRank() + smallType.rank;
...
void SemisimpleSubalgebras::writeHCandidatesForOneOrbit(
  ...
  targetSubalgebraWithLastHMissing.computeCentralizerOfOldHs(
    centralizerOfOldHs
  );
  orbitBodyStream
  << "sl(2) orbit "
  ...
  << " out of "
  << targetSubalgebraWithLastHMissing.getAmbientWeyl().rootsOfBorel.size
  << ". ";
  orbitHeader.report(orbitBodyStream.str());
  ...
  while (currentOrbit.incrementReturnFalseIfPastLast()) {
    this->writeOneHCandidate(
      ...
      outputHCandidatesScaledToActByTwo,
      targetSubalgebraWithLastHMissing,
      currentRootInjection,
      centralizerOfOldHs
    );
    totalHCandidates ++;
...
void SemisimpleSubalgebras::writeOneHCandidate(
  ...
  if (
    !targetSubalgebraWithLastHMissing.isGoodHNewActingByTwo(
  ...
  while (simpleSummandSelection.incrementReturnFalseIfPastLast()) {
    ...
    if (
      ...
      if (
        ...
        << "subalgebras of the original summand, which are: "
...
void PossibleExtensionsOfSemisimpleLieSubalgebra::computeCurrentHCandidates() {
  ...
  if (report0.tickAndWantReport()) {
    ...
    << ": "
    << this->state.numberOfExtensionTypesExplored + 1
    << " out of "
...
void PossibleExtensionsOfSemisimpleLieSubalgebra::initializeIfNeeded() {
  ...
  List<DynkinType> dynkinTypes;
  List<List<int> > injections;
  this->owner->growDynkinType(
    this->realizedBase->content.weylNonEmbedded->dynkinType,
    dynkinTypes,
    &injections
  );
  this->state.possibleExtensionTypes.clear();
  for (int i = 0; i < dynkinTypes.size; i ++) {
    DynkinTypeExtension extension;
    extension.dynkinType = dynkinTypes[i];
    extension.rootInjection = injections[i];
    this->state.possibleExtensionTypes.addOnTop(extension);
  }
...
nextUnexploredDynkinType() {
  ...
  return
  this->state.possibleExtensionTypes[
    this->state.numberOfExtensionTypesExplored
  ].dynkinType;
}
...
std::string PossibleExtensionsOfSemisimpleLieSubalgebra::toString() {
  ...
  << ": &nbsp;&nbsp;&nbsp;&nbsp;";
  for (int j = 0; j < this->state.possibleExtensionTypes.size; j ++) {
    if (j == this->state.numberOfExtensionTypesExplored) {
      out << "<b>";
    }
    out << this->state.possibleExtensionTypes[j].dynkinType.toString();
    if (j == this->state.numberOfExtensionTypesExplored) {
      out << "</b>";
    }
    if (j != this->state.possibleExtensionTypes.size - 1) {
      out << ", ";
    }
  }
  out
  ...
  if (
    this->state.numberOfExtensionTypesExplored < this->state.
    possibleExtensionTypes.size
  ) {
...
void CandidateSemisimpleSubalgebra::attemptToSolveSystem(
  ...
  if (currentExtension != nullptr) {
    ...
    this->indexInducedFrom = currentExtension->realizedBase->indexInOwner;
  }
...
void CandidateSemisimpleSubalgebra::computeInvolvedGeneratorsOfIndex(
  ...
  STACK_TRACE(
    "CandidateSemisimpleSubalgebra::computeInvolvedGeneratorsOfIndex"
  );
  List<ChevalleyGenerator>& involvedPositiveGenerators =
  ...
  involvedPositiveGenerators.setExpectedSize(
    this->getAmbientWeyl().getDimension() * 2
  );
...
bool CandidateSemisimpleSubalgebra::candidateInvolvesAllowedGeneratorsOnly(
  ElementSemisimpleLieAlgebra<AlgebraicNumber>& candidate,
  List<ChevalleyGenerator>& allowedSummands
) {
  STACK_TRACE(
    "CandidateSemisimpleSubalgebra::candidateInvolvesAllowedGeneratorsOnly"
  );
  for (ChevalleyGenerator& summand : candidate.monomials) {
    if (!allowedSummands.contains(summand)) {
      return false;
    }
  }
  return true;
...
bool CandidateSemisimpleSubalgebra::induceUnknownsFromBaseSubalgebra(
  int indexCurrentRoot, int indexNewRoot
) {
  STACK_TRACE(
    "CandidateSemisimpleSubalgebra::induceUnknownsFromBaseSubalgebra"
  );
  if (
    !this->flagUsedInducingSubalgebraRealization ||
    this->indexInducedFrom == - 1
  ) {
    // Not using the subalgebra being extended to higher rank.
    return false;
  }
  if (indexCurrentRoot == indexNewRoot) {
    // This is the only root not present subalgebra being extended
    return false;
  }
  CandidateSemisimpleSubalgebra& inducer =
  this->owner->subalgebras.values[this->indexInducedFrom].content;
  if (inducer.status != CandidateSubalgebraStatus::realized) {
    // The subalgebra from which we are inducing has not been realized yet.
    return false;
  }
  int preimageIndex =
  DynkinType::getIndexPreimageFromRootInjection(
    indexCurrentRoot, this->rootInjectionsFromInducer
  );
  Vector<Rational> inducerCartanElement =
  inducer.cartanElementsScaledToActByTwo[preimageIndex];
  Vector<Rational> targetCartanElement =
  this->cartanElementsScaledToActByTwo[indexCurrentRoot];
  if (inducerCartanElement != targetCartanElement) {
    // The cartan element of the inducing subalgebra does
    // not equal the desired Cartan element.
    // This is possible if the two are different conjugates
    // in the same Weyl orbit.
    // Recall that to guarantee uniqueness of our subalgebras,
    // we act with the Weyl group on all elements of the cartan simultaneously
    // which can shuffle the order of the vectors.
    return false;
  }
  ElementSemisimpleLieAlgebra<AlgebraicNumber>& negativeCandidate =
  inducer.negativeGenerators[preimageIndex];
  ElementSemisimpleLieAlgebra<AlgebraicNumber>& positiveCandidate =
  inducer.positiveGenerators[preimageIndex];
  if (
    !this->candidateInvolvesAllowedGeneratorsOnly(
      negativeCandidate, this->involvedNegativeGenerators[indexCurrentRoot]
    ) ||
    !this->candidateInvolvesAllowedGeneratorsOnly(
      positiveCandidate, this->involvedPositiveGenerators[indexCurrentRoot]
    )
  ) {
    // The candidate involves chevalley generators that don't have
    // the right weights with respect to the Cartan.
    return false;
  }
  // The Cartan element of the inducing subalgebra equals
  // the Cartan element of the current subalgebra.
  // We can reuse the realizations of the inducing subalgebra.
  this->unknownNegativeGenerators[indexCurrentRoot] = negativeCandidate;
  // <-implicit type conversion from base field to polynomial here
  this->unknownPositiveGenerators[indexCurrentRoot] = positiveCandidate;
  // <-implicit type conversion from base field to polynomial here
  return true;
}
...
bool CandidateSemisimpleSubalgebra::prepareSystem(
  ...
  for (int i = 0; i < this->involvedNegativeGenerators.size; i ++) {
    if (
      ...
      this->flagUsedInducingSubalgebraRealization &&
      this->induceUnknownsFromBaseSubalgebra(i, indexNewRoot)
    ) {
...
void CandidateSemisimpleSubalgebra::attemptToSolveSystemPart2(
  ...
  if (
    ...
    // We have used generators from the inducing subalgebra.
    // This has the chance of accidentally making the serre relations
    // unsolvable - please figure out the mathematical details of why
    // and document here.
    // Therefore, let's solve the system again without that optimization.
    this->flagUsedInducingSubalgebraRealization = false;
    this->attemptToSolveSystemPart2(
      attemptToChooseCentalizer, allowNonPolynomialSystemFailure
    );
...
bool CandidateSemisimpleSubalgebra::checkDimensionOfSubalgebra(
  ...
  if (!allowNonPolynomialSystemFailure) {
    ...
    << this->weylNonEmbedded->dynkinType.getLieAlgebraDimension()
    << ". "
    << "The subalgebras generated is: <br>"
    << this->basis.toStringCommaDelimited()
    << "<br>"
    << "The subalgebra is "
    << this->toString(nullptr)
...
std::string RealizedSemisimpleSubalgebra::toString(FormatExpressions* format)
const {
...
bool CandidateSemisimpleSubalgebra::compareLeftGreaterThanRight(
  ...
  if (rightSemisimplePart > leftSemisimplePart) {
    return false;
  }
...
computePrimalModuleDecompositionHighestWeightsLastPart() {
  ...
  for (
    ...
    std::stringstream mathMLStream;
    if (i < rank) {
      currentStream << "\\omega_{" << i + 1 << "}";
      mathMLStream << "<msub><mi>&omega;</mi><mn>" << i + 1 << "</mn></msub>";
    } else {
      currentStream << "\\psi";
      if (this->cartanOfCentralizer.size > 1) {
        mathMLStream << "<msub><mi>&psi;</mi><mn>" << i + 1 << "</mn></msub>";
...
std::string SemisimpleSubalgebras::toStringAlgebraLink(
  ...
  std::stringstream out;
  bool makeLink = format ==
  nullptr ? false : format->flagCandidateSubalgebraShortReportOnly;
  std::string typeString =
  this->subalgebras.values[actualindexSubalgebra].content.weylNonEmbedded->
  dynkinType.toMathMLFinal(nullptr);
  if (makeLink) {
    out
    << "<a href='"
...
std::string CandidateSemisimpleSubalgebra::toStringCartanSubalgebra(
  ...
  for (
    ...
    if (useLaTeX && useHtml) {
      out << simpleTypes[i].toMathMLFinal(&currentFormat) << ": ";
    } else {
...
std::string CandidateSemisimpleSubalgebra::toStringCentralizer(
  ...
  if (this->flagCentralizerIsWellChosen && this->centralizerRank != 0) {
    ...
    if (this->indexSemisimplePartCentralizer != - 1) {
      ...
      << this->owner->toStringAlgebraLink(
        this->indexSemisimplePartCentralizer, format
      );
    ...
    if (dimensionToralPartCentralizer != 0) {
      ...
      << "}";
      std::stringstream toralPartMathML;
      toralPartMathML
      << "<msub><mi>T</mi><mn>"
      << dimensionToralPartCentralizer.toString()
      << "</mn>"
      ...
      << MathML::toMathMLFinal(
        toralPartMathML.str(), toralPartStream.str(), format
      );
  ...
  if (
    ...
    << this->owner->toStringAlgebraLink(
      this->getSemisimplePartCentralizerOfSemisimplePartCentralizer(), format
    );
  }
  if (!this->flagCentralizerIsWellChosen) {
...
std::string CandidateSemisimpleSubalgebra::toStringSystemPart2(
  ...
  std::stringstream out;
  out
  << "<br><b>For the calculator "
  << "(equation order: mixed relations first):</b><br>\n"
  << this->configuredSystemToSolve.toStringCalculatorInputFromSystem(
    this->systemToSolveMixedRelationsFirst
  )
  << ";"
  << "<br> ";
  out << this->toStringLoadUnknown();
  if (this->flagUsedBuiltInRealization) {
    out
    << "<br> <b style='color:green'>Using built-in realization hints.</b> ";
  }
  out << this->toStringSubSystems();
  out << this->toStringInvolvedPositiveRoots() << "<br>";
  return out.str();
}

std::string CandidateSemisimpleSubalgebra::toStringInvolvedPositiveRoots()
const {
  std::stringstream out;
  out << "<br>Involved positive roots: ";
  for (int i = 0; i < this->involvedPositiveRoots.size; i ++) {
    const List<Vector<Rational> >& currentRoots =
    this->involvedPositiveRoots[i];
    out
    << "<br> Generator "
    << i + 1
    << " ("
...
std::string CandidateSemisimpleSubalgebra::toStringType(
  ...
  return this->weylNonEmbedded->dynkinType.toMathMLFinal(format);
}
...
std::string CandidateSemisimpleSubalgebra::toStringGenerators(
  ...
  for (int i = 0; i < this->negativeGenerators.size; i ++) {
    out << this->negativeGenerators[i].toMathMLFinal(format);
  ...
  for (int i = 0; i < this->positiveGenerators.size; i ++) {
    out << this->positiveGenerators[i].toMathMLFinal(format);
...
std::string CandidateSemisimpleSubalgebra::toString(FormatExpressions* format)
const {
  ...
  out
  << "Subalgebra type: "
  << this->owner->toStringAlgebraLink(this->indexInOwner, format)
  ...
  if (this->indexInducedFrom != - 1) {
    ...
    << this->owner->toStringAlgebraLink(this->indexInducedFrom, format)
    << ". ";
  }
  if (!this->flagCentralizerIsWellChosen) {
    out
  ...
  if (this->indicesDirectSummandSuperAlgebra.size > 0) {
    ...
    for (int i = 0; i < this->indicesDirectSummandSuperAlgebra.size; i ++) {
      out
      << this->owner->toStringAlgebraLink(
        this->indicesDirectSummandSuperAlgebra[i], format
      );
  ...
  out << this->weylNonEmbedded->cartanSymmetric.toMathMLFinal(&currentFormat);
  out
  << "<br>Scalar products of elements of "
  << "Cartan subalgebra scaled to act by 2 "
  << "(co-symmetric Cartan matrix): ";
  out
  << this->weylNonEmbedded->coCartanSymmetric.toMathMLFinal(&currentFormat);
  ...
  out << "<hr>Decomposition of ambient Lie algebra: ";
  out
  << this->characterNonPrimalFundamentalCoordinates.toMathMLFinal(
  ...
  if (this->cartanOfCentralizer.size > 0) {
    ...
    out << this->primalCharacter.toMathMLFinal(&characterFormatNonConstant);
  }
...
void CandidateSemisimpleSubalgebra::adjustCentralizerAndRecompute(
  ...
  this->attemptToSolveSystem(true, allowNonPolynomialSystemFailure, nullptr);
  this->computeCentralizerIsWellChosen();
}
...
bool DynkinType::operator>(const DynkinType& other) const {
  ...
  Rational maxComponentDifferenceMultiplicity = difference.coefficients[0];
  for (int i = 1; i < difference.size(); i ++) {
    if (difference[i] > highestSimpleTypeDifference) {
      maxComponentDifferenceMultiplicity = difference.coefficients[i];
      highestSimpleTypeDifference = difference[i];
    }
  }
  return maxComponentDifferenceMultiplicity > 0;
}
...
void CandidateSemisimpleSubalgebra::computeCartanOfCentralizer() {
  ...
  Matrix<Rational> diagonalMatrix2;
  ...
  for (int i = 0; i < this->bilinearFormSimplePrimal.numberOfRows; i ++) {
    if (i < this->cartanElementsScaledToActByTwo.size) {
      ...
      /
      2;
      diagonalMatrix2(i, i) = this->weylNonEmbedded->cartanSymmetric(i, i) / 2;
    } else {
      diagonalMatrix(i, i).assignNumeratorAndDenominator(1, 2);
      diagonalMatrix2(i, i) = 1;
    }
  }
  ...
  this->matrixMultiplyFundamentalCoordinatesToGetSimple *= diagonalMatrix2;
