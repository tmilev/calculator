...
std::string SemisimpleSubalgebras::toStringSemisimpleSubalgebrasSummaryLaTeX(FormatExpressions* format) const {
  ...
  if (totalBadParabolics > 0) {
    out << "<br><span style='color:red'>There are " << totalBadParabolics << " bad parabolic subalgebras!</span><br>";
  }
  ...
  if (numNoLinfRelFound == 0) {
    out << "<br>In all " << numFailingConeCondition
  ...
  for (int i = 0; i < this->subalgebras.values.size; i ++) {
    ...
    out << "&$" << currentSubalgebra.characterNonPrimalFundamentalCoordinates.toString(&characterFormat) << "$ ";
    out << "&$" << currentSubalgebra.primalCharacter.toString(&characterFormat) << "$ ";
    out << "& " << currentSubalgebra.fernandoKacNilradicalCandidates.size
...
bool SemisimpleSubalgebras::loadState(
  ...
  if (currentChainIntegers.size != numberOfExploredTypes.size || currentChainIntegers.size != numberOfExploredHs.size) {
    reportStream << "<hr>Input state is corrupt: currentChainInt.size: "
    << currentChainIntegers.size << ", numExploredTypes.size: "
  ...
  for (int i = 0; i < currentChainIntegers.size; i ++) {
    ...
    if (currentChainIntegers[i] < 0 || currentChainIntegers[i] >= this->subalgebras.values.size) {
      reportStream << "<hr>Corrupt subalgebra index: " << currentChainIntegers[i];
      return false;
    }
    CandidateSemisimpleSubalgebra& currentSubalgebra = this->subalgebras.values[currentChainIntegers[i]];
...
bool OrbitIteratorRootActionWeylGroupAutomorphisms::checkConsistency() {
  if (this->flagOrbitIsBuffered) {
    if (this->currentIndexInBuffer >= this->orbitBuffer.size) {
      global.fatal << "Current buffer index is: "
      << this->currentIndexInBuffer << " but the orbit has "
  ...
  if (this->flagOrbitEnumeratedOnce) {
    if (this->currentIndexInBuffer >= this->orbitSize) {
      global.fatal << "Current buffer index is: "
      << this->currentIndexInBuffer << " but the orbit was computed to have "
...
bool CandidateSemisimpleSubalgebra::checkElementSemisimpleLieAlgebra(
  ...
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::checkElementSemisimpleLieAlgebra");
...
std::string SemisimpleSubalgebras::WConjecture::toStringElementSemisimpleLieAlgebraOrMatrix(
  ...
  if (hasMatrixForm) {
    out << "<br>Matrix form:<br>"
    << matrixFormStream.str();
  }
  return out.str();
}
...
void CandidateSemisimpleSubalgebra::computeRatioKillingsByComponent() {
  ...
  for (int i = 0; i < this->cartanSubalgebrasByComponentScaledToActByTwo.size; i ++) {
    ...
    this->getAmbientSemisimpleLieAlgebra().getAdjoint(adMatrix, currentElement);
    adMatrix *= adMatrix;
...
void CandidateSemisimpleSubalgebra::WConjecture::compute(
  ...
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::WConjecture::compute");
  List<ElementSemisimpleLieAlgebra<AlgebraicNumber> > hWrapper;
  hWrapper.addOnTop(ownerSl2.hAlgebraic);
  owner.owner->owner->getCommonCentralizer(hWrapper, this->basisCentralizerOfH);
  ...
  for (int i = 0; i < this->modules.size; i ++) {
    for (int k = 0; k < this->modules[i].size; k ++) {
      ...
      for (int j = 0; j < this->modules[i][k].size; j ++) {
        ElementSemisimpleLieAlgebra<AlgebraicNumber>& currentElement = this->modules[i][k][j];
        Vector<Rational> currentRoot = this->getAmbientSemisimpleLieAlgebra().getWeightOfGenerator(currentElement[0].generatorIndex);
...
void CandidateSemisimpleSubalgebra::computePairKWeightElementAndModule(
  const ElementSemisimpleLieAlgebra<AlgebraicNumber>& leftKWeightElement, int rightIndex, List<int>& output
) {
...
void CandidateSemisimpleSubalgebra::computeKsl2TriplesGetOppositeElementsInOppositeModule(
  ...
  for (int i = 0; i < inputOppositeModule.size; i ++) {
    if ((this->getPrimalWeightFirstGenerator(inputOppositeModule[i]) + weight).isEqualToZero()) {
      outputElements.addOnTop(inputOppositeModule[i]);
    }
  }
}

Vector<Rational> CandidateSemisimpleSubalgebra::getPrimalWeightFirstGenerator(const ElementSemisimpleLieAlgebra<AlgebraicNumber>& input) const {
    ...
    for (int j = 0; j < this->weightsModulesPrimal[i].size; j ++) {
      if ((weight + this->weightsModulesPrimal[i][j]).isEqualToZero()) {
        for (int k = 0; k < this->modules[i].size; k ++) {
          ...
          if (!(weight + this->getPrimalWeightFirstGenerator(this->modules[i][k][j])).isEqualToZero()) {
...
void CandidateSemisimpleSubalgebra::computePrimalModuleDecompositionHighestWeights(
...
void CandidateSemisimpleSubalgebra::computePrimalModuleDecompositionHighestWeightsLastPart() {
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::computePrimalModuleDecompositionHighestWeightsLastPart");
  ...
  for (int i = 0; i < this->highestVectorsNonSorted.size; i ++) {
    ...
    Vector<Rational>& currentHighestWeightRelative = this->highestWeightsNonPrimalNonSorted[i];
    Vector<Rational>& currentHighestWeightPrimal = this->highestWeightsPrimalNonSorted[i];
    this->getWeightProjectionFundamentalCoordinates(currentRoot, currentHighestWeightRelative);
    this->getPrimalWeightProjectionFundamentalCoordinates(currentRoot, currentHighestWeightPrimal);
  ...
  for (int i = 0; i < modulesComputer.size; i ++) {
    ...
    if (LinearCombination<ChevalleyGenerator, Rational>::vectorSpacesIntersectionIsNonTrivial(modulesComputer[i], this->basis)) {
      ...
      if (this->highestVectors.lastObject()->size != 1) {
        ...
        << modulesComputer[i].toStringCommaDelimited() << " and "
        << this->basis.toStringCommaDelimited() << "."
        << global.fatal;
      }
...
void CandidateSemisimpleSubalgebra::computePrimalModuleDecompositionHighestWeightVectors(
  ...
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::computePrimalModuleDecompositionHighestWeightVectors");
  this->checkConsistency();
...
bool CandidateSemisimpleSubalgebra::attemptToSolveSystem() {
  ...
  ProgressReport report;
  std::stringstream reportstream;
  reportstream << "<hr>In order to realize type "
  << this->weylNonEmbedded->dynkinType.toString()
...
void CandidateSemisimpleSubalgebra::getGenericNegativeGeneratorLinearCombination(
  int indexNegativeGenerators, ElementSemisimpleLieAlgebra<Polynomial<AlgebraicNumber> >& output
) {
  int offsetIndex = 0;
...
void CandidateSemisimpleSubalgebra::getGenericCartanCentralizerLinearCombination(
  ...
  int offsetIndex = this->totalNumUnknownsNoCentralizer + indexCartanCentralizerGenerator * this->getAmbientWeyl().getDimension() ;
  ...
  for (int i = 0; i < elementsOfCartan.size; i ++) {
    elementsOfCartan[i].generatorIndex = this->getAmbientSemisimpleLieAlgebra().getNumberOfPositiveRoots() + i;
    elementsOfCartan[i].owner = &this->getAmbientSemisimpleLieAlgebra();
  }
  this->getGenericLinearCombination(this->totalNumUnknownsWithCentralizer, offsetIndex, elementsOfCartan, output);
}
...
void CandidateSemisimpleSubalgebra::getGenericLinearCombination(
  ...
  for (int i = 0; i < involvedGenerators.size; i ++) {
    ...
    monomial.makeGenerator(involvedGenerators[i].generatorIndex, *this->owner->owner);
...
bool CandidateSemisimpleSubalgebra::computeCharacter(bool allowBadCharacter) {
  if (this->indexNonEmbeddedMeStandard == - 1) {
    ...
    << "has not been initialized properly. " << global.fatal;
  }
      ...
      for (int j = 0; j < this->cartanSubalgebrasByComponentScaledToActByTwo[i].size; j ++) {
        ...
        if (!tempMon.weightFundamentalCoordinates[counter].isInteger()) {
          if (!allowBadCharacter) {
            ...
            << "possibility has been explicitly selected. " << global.fatal;
          }
          return false;
        }
      }
...
std::string SlTwoSubalgebra::toString(FormatExpressions* format) const {
  ...
  std::stringstream out;
  std::string tempS;
  out << "<a name ='sl2index" << this->indexInContainer
  << "'>h-characteristic: " << this->hCharacteristic.toString() << "</a>";
...
void SlTwoSubalgebra::computeModuleDecompositionsition(
  ...
  MacroRegisterFunctionWithName("SlTwoSubalgebra::computeModuleDecompositionsition");
  this->checkConsistency();
  ...
  for (int j = bufferHighestWeights.size - 1; j >= indexZeroWeight; j --) {
    ...
    if (topMult > 0) {
      currentHighestWeight.weightFundamentalCoordinates[0] = j - indexZeroWeight;
...
bool CandidateSemisimpleSubalgebra::isExtremeWeight(int moduleIndex, int indexInIsomorphismComponent) const {
  Vector<Rational>& currentWeight = this->weightsModulesPrimal[moduleIndex][indexInIsomorphismComponent];
...
std::string CandidateSemisimpleSubalgebra::toStringDrawWeightsHelper(int indexModule, const Vector<Rational>& weight) const {
  ...
  std::stringstream out;
  List<List<ElementSemisimpleLieAlgebra<AlgebraicNumber> > >& currentModule = this->modules[indexModule];
  ...
  for (int i = 0; i < currentModule.size; i ++) {
    ...
    for (int j = 0; j < currentModule[i].size; j ++) {
      ...
      out << "<span class =\\\"math\\\">"
      << HtmlRoutines::doubleBackslashes(currentModule[i][j].toString()) << "</span>";
      out << closeTag << "</td>";
...
std::string CandidateSemisimpleSubalgebra::toStringDrawWeights(FormatExpressions* format) const {
  ...
  int primalRank = - 1;
  (this->centralizerRank + this->cartanElementsSubalgebra.size).isSmallInteger(&primalRank);
  if (primalRank < 2) {
    return "";
  }
  std::stringstream out;
  if (primalRank != this->bilinearFormFundamentalPrimal.numberOfColumns) {
    out << "<br>The primal rank was computed to be " << primalRank
  ...
  for (int i = 0; i < primalRank; i ++) {
    for (int j = 0; j < primalRank; j ++) {
      drawingVariables.operations.bilinearForm(i, j) = this->bilinearFormFundamentalPrimal(i, j).getDoubleValue();
    }
  }
  ...
  for (int i = 0; i < this->modules.size; i ++) {
    largestModuleDimension = MathRoutines::maximum(this->modules[i][0].size, largestModuleDimension);
  }
  drawingVariables.defaultHtmlHeight = MathRoutines::maximum(600, largestModuleDimension * 35);
  for (int i = 0; i < this->modules.size; i ++) {
      ...
      for (int k = 0; k < this->modules[i][j].size; k ++) {
        if (j == 0) {
          ...
          if (this->primalSubalgebraModules.contains(i)) {
            drawingVariables.drawLineBetweenTwoVectorsBufferRational(zeroVector, this->weightsModulesPrimal[i][k], "green");
          }
          drawingVariables.drawCircleAtVectorBufferRational(this->weightsModulesPrimal[i][k], color, 2);
          if (this->isExtremeWeight(i, k)) {
            cornerWeights.addOnTop(this->weightsModulesPrimal[i][k]);
          }
        }
        if (k == this->modules[i][j].size - 1 && basisToDrawCirclesAt.size < primalRank) {
    ...
    for (int j = 0; j < cornerWeights.size; j ++) {
      ...
      for (int k = j + 1; k < cornerWeights.size; k ++) {
        if (minimalDistance == Vector<Rational>::scalarProduct(
          cornerWeights[k] - cornerWeights[j],
          cornerWeights[k] - cornerWeights[j],
          this->bilinearFormFundamentalPrimal
        )) {
          drawingVariables.drawLineBetweenTwoVectorsBufferRational(cornerWeights[k], cornerWeights[j], color, 1);
        }
      }
    }
  }
  ...
  for (int i = 0; i < this->modules.size; i ++) {
    ...
    drawingVariables.operations.drawHighlightGroup(highlightGroup, highlightLabels, "black", 5);
  }
  drawingVariables.operations.basisToDrawCirclesAt.setSize(basisToDrawCirclesAt.size);
  for (int i = 0; i < drawingVariables.operations.basisToDrawCirclesAt.size; i ++) {
    drawingVariables.operations.basisToDrawCirclesAt[i].setSize(primalRank);
    for (int j = 0; j < primalRank; j ++) {
      drawingVariables.operations.basisToDrawCirclesAt[i][j] = basisToDrawCirclesAt[i][j].getDoubleValue();
    }
    drawingVariables.drawCircleAtVectorBufferRational(basisToDrawCirclesAt[i], "red", 4);
  }
...
std::string CandidateSemisimpleSubalgebra::toStringModuleDecompositionLaTeX(FormatExpressions* format) const {
  ...
  for (int i = 0; i < this->modules.size; i ++) {
    int numberOfRowsIsotypicComposition = this->modules[i].size * this->modules[i][0].size;
    ...
    for (int j = 0; j < this->modules[i].size; j ++) {
      ...
      out << "{$V_{" << this->highestWeightsPrimal[i].toStringLetterFormat("\\omega", &characterFormat) << "}$} ";
      out << "&";
      for (int k = 0; k < this->modules[i][j].size; k ++) {
        if (k > 0) {
          out << "&&";
        ...
        out << "&";
        out << "$" << this->weightsModulesPrimal[i][k].toStringLetterFormat("\\omega", &characterFormat) << "$" << "& ";
...
std::string CandidateSemisimpleSubalgebra::toStringModuleDecomposition(FormatExpressions* format) const {
  ...
  for (int i = 0; i < this->highestWeightsPrimal.size; i ++) {
    ...
    if (visible) {
      ...
      std::stringstream tempStream;
      tempStream << "V_{"
      << this->highestWeightsPrimal[i].toStringLetterFormat("\\omega", &characterFormat)
      << "} ";
      out << HtmlRoutines::getMathNoDisplay(tempStream.str())
      << " &rarr; " << this->highestWeightsPrimal[i].toString();
      out << "</td>";
    }
  }
  out << "</tr>";
  ...
  out << "</tr>";
  out << "<tr>";
  out << "<td>Weights of elements in fundamental coords w.r.t. "
  ...
  for (int i = 0; i < this->weightsModulesNONprimal.size; i ++) {
    ...
    for (int j = 0; j < this->weightsModulesNONprimal[i].size; j ++) {
      out << "\\("
      << this->weightsModulesNONprimal[i][j].toStringLetterFormat("\\omega", &characterFormat)
      << "\\)";
  ...
  for (int i = 0; i < this->weightsModulesPrimal.size; i ++) {
    ...
    for (int j = 0; j < this->weightsModulesPrimal[i].size; j ++) {
      out << "\\("
      << this->weightsModulesPrimal[i][j].toStringLetterFormat("\\omega", &characterFormat)
      << "\\)";
  ...
  if (this->charactersPrimalModules.size > 0) {
    for (int i = 0; i < this->charactersPrimalModules.size; i ++) {
      out << "<td>";
      out << HtmlRoutines::getMathNoDisplay(this->charactersPrimalModules[i].toString(&characterFormat));
      out << "</td>";
    }
  } else {
    for (int i = 0; i < this->modules.size; i ++) {
  ...
  if (this->charactersPrimalModulesMerged.size > 0) {
    for (int i = 0; i < this->charactersPrimalModulesMerged.size; i ++) {
      out << "<td>";
      out << HtmlRoutines::getMathNoDisplay(this->charactersPrimalModulesMerged[i].toString(&characterFormat));
      out << "</td>";
    }
  } else {
    for (int i = 0; i < this->modules.size; i ++) {
...
std::string NilradicalCandidate::toString(FormatExpressions* format) const {
  ...
  out << "<br> Highest weight cone:<br> "
  << this->toStringTableElementWithWeights(this->nonFernandoKacHighestWeightVectors, this->nonFernandoKacHighestWeights);
  if (this->flagNilradicalConesIntersect) {
    ...
    if (this->flagNilradicalConesStronglyIntersect) {
      ...
      for (int i = 0; i < this->nilradicalWeights.size; i ++) {
        if (this->coneStrongIntersection[i] != 0) {
          Vector<Rational>& currentNilradicalWeight = this->nilradicalWeights[i];
          out << "<td><table border =\"1\"><tr><td>" << currentNilradicalWeight.toString() << "</td></tr>";
          for (int j = 0; j < this->nilradicalWeights.size; j ++) {
          ...
          << this->owner->getScalarSubalgebra(currentNilradicalWeight, this->getConeStrongIntersectionWeight())
          << "</td></td>";
          out << "</table>";
          out << "</td>";
        }
...
void NilradicalCandidate::getModGeneratedByNonHighestWeightVectorAndNilradElement(
  ...
  for (int i = 0; i < outputBrackets.size; i ++) {
    if (!outputBrackets[i].isEqualToZero()) {
      for (int j = 0; j < this->nonFernandoKacHighestWeightVectorsStrongRelativeToSubset.size; j ++) {
        this->owner->owner->owner->lieBracket(this->nonFernandoKacHighestWeightVectorsStrongRelativeToSubset[j], outputBrackets[i], lieBracket);
...
std::string CandidateSemisimpleSubalgebra::toStringNilradicalsSummary(FormatExpressions* format) const {
  ...
  if (this->numberOfConeIntersections > 0) {
    if (this->numberOfCasesNoLInfiniteRelationFound > 0) {
      out << "<br><span style='color:red'>In " << this->numberOfCasesNoLInfiniteRelationFound
      << " cases no L-infinite relation was found. </span>";
    } else {
      out << "<br><span style='color:red'> In each of " << this->numberOfConeIntersections
      << " case(s) of intersecting cones, an L-infinite relation was found. </span>";
    }
  }
  return out.str();
...
std::string CandidateSemisimpleSubalgebra::toStringNilradicals(FormatExpressions* format) const {
  ...
  if (this->owner->flagProduceLaTeXTables) {
    ...
    for (int i = 0; i < this->fernandoKacNilradicalCandidates.size; i ++) {
      const NilradicalCandidate& currentNilradical = this->fernandoKacNilradicalCandidates[i];
      ...
      if (currentNilradical.flagLinfiniteRelFound) {
        ...
        for (
          int j = 0;
          j < currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubsetWeights.size + currentNilradical.nilradicalSubsetWeights.size;
          j ++
        ) {
          out << "cc";
        }
        out << "}Relation&";
        for (int j = 0; j < currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubsetWeights.size; j ++) {
          Rational coefficient = currentNilradical.coneRelativelyStrongIntersection[currentNilradical.nilradicalSubsetWeights.size + j];
          coefficient.negate();
          out << "$"
          << (currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubsetWeights[j] * coefficient).toStringLetterFormat("\\omega", &tempFormat)
          << "$";
          out << " & ";
          if (j != currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubsetWeights.size - 1) {
            out << "+&";
          } else {
            out << "= &";
          }
        }
        for (int j = 0; j < currentNilradical.nilradicalSubsetWeights.size; j ++) {
          Rational coefficient = currentNilradical.coneRelativelyStrongIntersection[j];
          out << "$" << (currentNilradical.nilradicalSubsetWeights[j] * coefficient).toStringLetterFormat("\\omega", &tempFormat) << "$";
          out << " & ";
        ...
        out << "\\\\<br>\n Elts.& ";
        for (int j = 0; j < currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubset.size; j ++) {
          out << "$" << currentNilradical.nonFernandoKacHighestWeightVectorsStrongRelativeToSubset[j].toString() << "$";
          out << " & &";
        }
        ...
        for (int j = 0; j < currentNilradical.nilradicalSubset.size; j ++) {
          out << "$" << currentNilradical.nilradicalElementOpposites[currentNilradical.nilradicalSubselection.elements[j]].toString() << "$";
          out << " & ";
          if (j != currentNilradical.nilradicalSubset.size - 1) {
            out << " &";
          }
        ...
        std::stringstream tempStream;
        for (int j = 0; j < currentNilradical.nilradicalSubset.size; j ++) {
          currentNilradical.getModGeneratedByNonHighestWeightVectorAndNilradElement(j, relevantBracketsLeft, relevantBracketsRight, relevantBrackets);
          ...
          for (int k = 0; k < currentNilradical.nilradicalSubset.size; k ++) {
            if (k != j) {
              this->owner->owner->lieBracket(
                ...
                currentNilradical.nilradicalElementOpposites[currentNilradical.nilradicalSubselection.elements[k]],
                element
              );
              tempStream << "$[" << currentNilradical.nilradicalSubset[j].toString() << ", "
              << currentNilradical.nilradicalElementOpposites[currentNilradical.nilradicalSubselection.elements[k]].toString()
              << "] =" << element.toString() << ", ";
            }
          }
        }
  ...
  //    for (int j = 0; j < currentNilradical.nonFernandoKacHighestWeights.size; j ++)
  //    { out << " $" << currentNilradical.nonFernandoKacHighestWeights[j].toStringLetterFormat("\\omega", &tempFormat) << "$";
...
std::string CandidateSemisimpleSubalgebra::toStringSystemPart2(FormatExpressions* format) const {
  (void) format;//avoid unused parameter warning in a portable way
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::toStringSystemPart2");
  std::stringstream out;
...
std::string CandidateSemisimpleSubalgebra::toString(
  ...
  if (!shortReportOnly) {
    ...
    if (this->owner->flagProduceLaTeXTables) {
      out << "LaTeX version of pairing table: <br><br>"
      << this->toStringPairingTableLaTeX(format) << "<br><br>";
    }
...
void CandidateSemisimpleSubalgebra::getHsScaledToActByTwoByType(
  ...
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::getHsScaledToActByTwoByType");
...
bool CandidateSemisimpleSubalgebra::hasHsScaledByTwoConjugateTo(List<Vector<Rational> >& input) const {
  MacroRegisterFunctionWithName("CandidateSemisimpleSubalgebra::hasHsScaledByTwoConjugateTo");
...
bool CandidateSemisimpleSubalgebra::isDirectSummandOf(const CandidateSemisimpleSubalgebra& other) {
  ...
  this->weylNonEmbedded->dynkinType.getOuterAutosGeneratorsActOnVectorColumn(outerAutomorphisms.generators);
  ...
  if (report.tickAndWantReport()) {
    ...
    for (int i = 0; i < outerAutomorphisms.elements.size; i ++) {
      if (i >= 100) {
        ...
        reportStream << "<br>" << HtmlRoutines::getMathNoDisplay(outerAutomorphisms.elements[i].toStringMatrixForm(&format));
      }
    }
    report.report(reportStream.str());
  }
  do {
    for (int k = 0; k < outerAutomorphisms.elements.size; k ++) {
      ...
      outerAutomorphisms.elements[k].actOnVectorROWSOnTheLeft(conjugationCandidates, conjugationCandidates);
...
void SemisimpleSubalgebras::hookUpCentralizers(bool allowNonPolynomialSystemFailure) {
  ...
  for (int i = 0; i < this->subalgebras.values.size; i ++) {
    ...
    currentSubalgebra.indexInducedFrom = candidatePermutationHashed.getIndex(currentSubalgebra.indexInducedFrom);
    ...
    for (int j = 0; j < this->subalgebras.values.size; j ++) {
      ...
      CandidateSemisimpleSubalgebra& otherSubalgebra = this->subalgebras.values[j];
...
void CandidateSemisimpleSubalgebra::computeCartanOfCentralizer() {
  ...
  for (int i = 0; i < this->cartanOfCentralizer.size; i ++) {
    element.assignVectorNegativeRootSpacesCartanPosistiveRootSpaces(outputCartanCentralizer[i], *this->owner->owner);
