...
/*  this->AddNonBoundVarMustBeNew
  ...
  ("SolveNseparableQuasiLinearSystem", & this->fSolveSeparableBilinearSystem, "",
   ...
   in the first n variables with coefficients in the polynomial ring generated by the remaining m\
   variables. \
   The function takes as argument the list of polynomials of the system. The variables are ordered\
   in lexicographic order; suppose the variables are x_1,..., x_p. Then\
   The number n is extracted by defining it to be the largest index n>=1 such that \
   the system is linear in the variables x_1,..., x_n. ",
