...
public:
  ...
  friend std::ostream& operator<<(
    std::ostream& out, const ClassFunction<someFiniteGroup, Coefficient>& data
  ) {
    out << data.toString();
    return out;
  }
...
class ConjugationAction: public GroupActionInterface<
  ElementSomeGroup, ElementSomeGroup
> {
public:
  ConjugationAction() {
    this->name = "conjugation";
  }
  void actOn(
    const ElementSomeGroup& actingElement,
    const ElementSomeGroup& inputElementActedUpon,
    ElementSomeGroup& output
  ) {
    ElementSomeGroup::conjugationAction(
      actingElement, inputElementActedUpon, output
    );
  }
};
...
// bool hasDifferentConjugacyInvariantsFrom(const elementSomeGroup& other)
// const;
// //the purpose of this is to test if it is possible for two elements to be
// conjugate
// //return true if there is an easy way to tell if they are not conjugate
// //return false otherwise
// void getCharacteristicPolynomialStandardRepresentation(Polynomial<Rational>&
// out) const;
...
// unsigned int hashFunction() const;
//
// To make certain operations go faster, it might be nice to put smarter
// algorithms
...
public:
  ...
  // a particular FiniteGroup, it may require something pointed from
  // specificDataPointer
  // which should usually be a PermutationGroupData for
  // FiniteGroup<PermutationR2>
  // and so forth
  void* specificDataPointer;
  // It is occasionally useful to a group that is formed as a subgroup of
  // another
  // group to know about its relationship to its parent
  // Currently, 'occasionally useful' means the getWordByFormula method of
  // subgroups
  // that can use the TranslatableWordsSubgroupGetWordByFormula algorithm
  SubgroupData<FiniteGroup<elementSomeGroup>, elementSomeGroup>*
  parentRelationship;
  ...
  // insert using .sortedInsertDontDup() if you for some reason can't use the
  // friendly
  // insertion methods provided.
  List<GroupRepresentation<FiniteGroup<elementSomeGroup>, Rational> >
  ...
  // WARNING: This recalculates conjugacy classes, screwing up their order for
  // other methods
  ...
  bool(*areConjugateByFormula)(
    const elementSomeGroup& x, const elementSomeGroup& y
  );
  void(*computeConjugacyClassSizesAndRepresentativesByFormula)(
  ...
  bool(*getWordByFormula)(
    FiniteGroup<elementSomeGroup>& G, const elementSomeGroup& g, List<int>& out
  );
  ...
  // This method needs some initial irreps to be seeded in either the irreps or
  // the irreps_grcam locations
  void computeIrreducibleRepresentationsTodorsVersion();
  // In order for this to work, the starting irrep must be one whose n-fold
  // tensor products contain every other irrep
  void computeIrreducibleRepresentationsThomasVersion(
    GroupRepresentationCarriesAllMatrices<
      FiniteGroup<elementSomeGroup>, Rational
    >* startingIrrep
  );
  void(*ComputeIrreducibleRepresentationsWithFormulas)(
    FiniteGroup<elementSomeGroup>& G
  );
  ...
  SubgroupData<FiniteGroup<elementSomeGroup>, elementSomeGroup>
  parabolicKindaSubgroupGeneratorSubset(const List<int>& subgens);
  // auto M = G.getEmptyRationalRepresentation()
  GroupRepresentation<FiniteGroup<elementSomeGroup>, Rational>
  getEmptyRationalRepresentation();
  friend std::ostream& operator<<(
    std::ostream& out, FiniteGroup<elementSomeGroup>& data
  ) {
    out << data.toString(0);
    return out;
  }
...
struct SimpleReflectionOrOuterAutomorphism {
  ...
  static unsigned int hashFunction(
    const SimpleReflectionOrOuterAutomorphism& input
  ) {
...
class ElementWeylGroup {
  friend std::ostream& operator<<(
    std::ostream& output, const ElementWeylGroup& element
  ) {
...
public:
  ...
  void makeFromReadableReflections(
    WeylGroupData& input,
    bool dontmakeCanonical_SET_TRUE_ON_YOUR_OWN_RISK,
    const std::string& inputReflections
  );
  bool operator>(const ElementWeylGroup& other) const;
};
...
class WeylGroupData {
  ...
  void computeFundamentalToSimpleMatrices() {
    ...
    this->fundamentalToSimpleCoordinates.assignVectorsToRows(
      fundamentalBasis
    );
  ...
  friend std::ostream& operator<<(
    std::ostream& output, const WeylGroupData& data
  ) {
...
public:
  ...
  void makeFromDynkinTypeDefaultLengthKeepComponentOrder(
  ...
  void generateAdditivelyClosedSubset(
    Vectors<Rational>& input, Vectors<Rational>& output
  );
  bool loadGAPRootSystem(
    HashedList<Vector<Rational> >& outputPositiveRootSystem
  ) const;
  Rational getKillingDividedByTraceRatio();
  bool containsRootNonStronglyPerpendicularTo(
    Vectors<Rational>& vectors, Vector<Rational>& input
  );
  int numberOfRootsConnectedTo(
    Vectors<Rational>& vectors, Vector<Rational>& input
  );
  ...
  Vector<Coefficient> getDualCoordinatesFromFundamental(
    ...
    if (result.size != this->getDimension()) {
      ...
      << result.size
      << " coordinates, while the rank of the Weyl group is "
      << this->getDimension()
      << ". "
      << global.fatal;
    }
  ...
  template <class Coefficient>
  Coefficient getScalarProductSimpleRoot(
    const Vector<Coefficient>& input, int indexSimpleRoot
  ) const {
  ...
  void getSimpleReflectionMatrix(int indexSimpleRoot, Matrix<Rational>& output)
  const;
  ...
  template <class Coefficient>
  void getEpsilonCoordinates(
    const Vector<Coefficient>& input, Vector<Coefficient>& output
  ) {
    this->computeEpsilonMatrix();
    output = input;
    this->matrixSendsSimpleVectorsToEpsilonVectors.getElement().
    actOnVectorColumn(output, Coefficient::zero());
  }
  void getEpsilonCoordinates(
  ...
  Vector<Rational> applyReflectionList(
    const List<int>& rightReflectionsActFirst, const Vector<Rational>& vv
  ) const;
  template <class Coefficient>
  void actOn(
    const ElementWeylGroup& groupElement, Vector<Coefficient>& inputOutput
  ) const {
  ...
  template <class Coefficient>
  void actOnRhoModified(
    const ElementWeylGroup& groupElement, Vector<Coefficient>& vector
  ) const {
  ...
  template <class Coefficient>
  void actOnRhoModified(int indexOfWeylElement, Vector<Coefficient>& vector)
  const {
  ...
  // this function will instead carry out the transformation v -> s(v - rho) +
  // rho,
  // where rho is the half-sum of the positive roots.
  //
  // If minusRho and rhoAction are both set, then instead we do v -> s(v + rho)
  // - rho.
  void reflectSimple(
    int index, Vector<Rational>& root, bool rhoAction, bool useMinusRho
  );
  ...
  void getMatrixStandardRepresentation(
    const ElementWeylGroup& input, Matrix<Rational>& outputMatrix
  ) const;
  Matrix<Rational> getMatrixStandardRepresentation(
    const ElementWeylGroup& input
  ) const {
  ...
  bool isPositiveOrPerpWithRespectToH(
    const Vector<Rational>& input, const Vector<Rational>& hVector
  ) {
  ...
  double rootScalarCartanRoot(
    ...
    if (r1.size != r2.size || r1.size != this->getDimension()) {
      ...
      << " and "
      << r2
      << " and the rank of the Weyl group is "
      << this->getDimension()
      << ". ";
    ...
    for (int i = 0; i < this->getDimension(); i ++) {
      for (int j = 0; j < this->getDimension(); j ++) {
        result +=
        this->cartanSymmetric.elements[i][j].getDoubleValue() * r1[i] * r2[j];
      }
    }
    return result;
  }
  template <class LeftType, class RightType>
  LeftType rootScalarCartanRoot(
    const Vector<LeftType>& r1, const Vector<RightType>& r2
  ) const {
    LeftType result;
  ...
  // non-zero scalar product with Vectors<Rational> of the Vector<Rational>
  // system,
  // without changing the inputH-sign of any Vector<Rational> that had a
  // non-zero scalar product to begin with
  void perturbWeightToRegularWithRespectToRootSystem(
    const Vector<Rational>& inputH, Vector<Rational>& output
  );
  template <class Coefficient>
  bool isDominantWithRespectToGenerator(
  ...
  void transformToSimpleBasisGeneratorsWithRespectToH(
...
template <class leftType, class rightType>
void WeylGroupData::rootScalarCartanRoot(
  const Vector<leftType>& r1, const Vector<rightType>& r2, leftType& output
) const {
  if (r1.size != r2.size || r1.size != this->getDimension()) {
    global.fatal
    << "Attempt to get the scalar product of the weight "
    << r1
    << " (dimension "
    << r1.size
    << ") with the weight "
    ...
    << " (dimension "
    << r2.size
    << "), while the dimension of the ambient Weyl group is "
    << this->getDimension()
    << ". ";
    global.fatal << global.fatal;
  }
...
public:
  ...
  bool isElementWeylGroupOrOuterAutomorphisms(
    const MatrixTensor<Rational>& matrix
  );
  bool areMaximallyDominantGroupOuter(List<Vector<Rational> >& weights);
  bool checkInitialization() const;
  // Standard action of a Weyl group automorphism on a root element.
  // We define the action by defining the action of the generators.
  // If acting element is a reflection s_\alpha, then the action is given by
  // s_\alpha(v) = v- 2<\alpha, v>/<\alpha,\alpha> \alpha.
  // If the acting element is an outer automorphism, then the action is
  // given by the matrix of the automorphism.
  // The action is then extended by associativity to all automorphisms.
  template <class Coefficient>
  void actOnStandard(
...
template <class Coefficient>
void ElementWeylGroupAutomorphisms::actOnStandard(
  const ElementWeylGroupAutomorphisms& inputElement,
  const Vector<Coefficient>& inputVector,
  Vector<Coefficient>& output
) {
  inputElement.checkInitialization();
  inputElement.owner->actOnStandard(inputElement, inputVector, output);
}

template <class Coefficient>
void ElementWeylGroupAutomorphisms::actOnStandard(
  Vector<Coefficient>& inputOutput
) const {
  this->checkInitialization();
  this->owner->actOnStandard(*this, inputOutput, inputOutput);
}

template <class Coefficient>
void WeylGroupAutomorphisms::actOnStandard(
  const ElementWeylGroupAutomorphisms& groupElement,
  const Vector<Coefficient>& inputVector,
  Vector<Coefficient>& outputVector
) const {
  ...
  for (
    ...
    if (!currentGenerator.flagIsOuter) {
      ...
      if (!this->flagAllOuterAutosComputed) {
        ...
        << " does not have its outer automorphisms "
        << "computed at a place where it should. "
        << global.fatal;
      }
      this->outerAutomorphisms.generators[currentGenerator.index].
...
public:
  ...
  template <typename Coefficient>
  GroupRepresentation<FiniteGroup<elementFirstGroup>, Coefficient>
  pullbackRepresentation(
...
public:
  friend std::ostream& operator<<(
    std::ostream& output, const GroupRepresentation& unused
  ) {
    (void) unused;
  ...
  std::string identifyingString;
  // in Python, this would be an anonymous object
  GroupRepresentation() {
  ...
  bool verifyRepresentationExpensive() {
    ...
    for (int i = 0; i < this->ownerGroup->elements.size; i ++) {
      for (int j = 0; j < this->ownerGroup->elements.size; j ++) {
        if (
          repms[i] * repms[j] !=
          repms[
            this->ownerGroup->elements.getIndex(
    ...
    global.comments
    << "Expensive verification complete, this is indeed a representation\n";
    return true;
  }
  ...
  bool operator==(const GroupRepresentation<someGroup, Coefficient>& right)
  const {
  ...
  GroupRepresentation(
    const GroupRepresentationCarriesAllMatrices<someGroup, Coefficient>& in
  ) {
    this->generators = in.generators;
    this->character = in.character;
    this->flagCharacterIsComputed = in.flagCharacterIsComputed;
  }
  GroupRepresentationCarriesAllMatrices<someGroup, Coefficient> makeGRCAM()
  const {
  ...
  unsigned int hashFunction() const;
  static unsigned int hashFunction(
    const GroupRepresentation<someGroup, Coefficient>& in
  ) {
...
bool GroupRepresentation<someGroup, Coefficient>::checkInitialization() const {
  if (this->ownerGroup == 0) {
    global.fatal
    << "Group not initialized at a place where it should be."
    << global.fatal;
  }
  return true;
}
...
void GroupRepresentation<someGroup, Coefficient>::computeCharacter() const {
  ...
  for (int cci = 0; cci < this->ownerGroup->conjugacyClasses.size; cci ++) {
    ...
    this->ownerGroup->getWord(
      this->ownerGroup->conjugacyClasses[cci].representative, ccirWord
    );
...
template <typename someGroup, typename Coefficient>
bool GroupRepresentation<someGroup, Coefficient>::operator>(
  const GroupRepresentation<someGroup, Coefficient>& right
) const {
  if (!this->flagCharacterIsComputed) {
    this->computeCharacter();
  }
...
template <typename someGroup, typename Coefficient>
template <typename somestream>
somestream&GroupRepresentation<someGroup, Coefficient>::intoStream(
  somestream& out
) const {
  if (!this->flagCharacterIsComputed) {
    // this->computeCharacter();
...
JSData GroupRepresentation<someGroup, Coefficient>::toJSON() {
  ...
  if (this->flagCharacterIsComputed) {
    ...
    out["character"].elementType = JSData::Type::tokenNull;
  }
...
public:
  ...
  // Coefficient>& other) const;
};

template <class somegroup, class Coefficient>
class VirtualRepresentation: public LinearCombination<
  ClassFunction<somegroup, Coefficient>, Rational
> {
public:
  ...
  static unsigned int hashFunction(
    const VirtualRepresentation<somegroup, Coefficient>& input
  ) {
    return input.hashFunction();
  }
  unsigned int hashFunction() const {
...
// The GroupAction is expected to inherit the GroupActionInterface.
template <class ElementGroup, class ElementRepresentation, class GroupAction>
class OrbitIterator {
private:
  HashedList<ElementRepresentation> privateLayer1;
  HashedList<ElementRepresentation> privateLayer2;
  HashedList<ElementRepresentation> privateLayer3;
...
public:
  ...
  void setGroupAction(const GroupAction& inputGroupAction) {
    this->groupAction = inputGroupAction;
  }
  bool checkInitialization() const;
  ...
  // Initializes the orbit generation.
  // You must also separately set the group action.
  void initialize(
...
std::ostream& operator<<(
  std::ostream& out, const ClassFunction<WeylGroupData, Coefficient>& data
);
template <typename elementSomeGroup>
class Coset {
public:
...
public:
  ...
  template <typename Coefficient>
  void quotientGroupPermutationRepresentation(
    GroupRepresentation<someGroup, Coefficient>& out
  );
  template <typename Coefficient>
  GroupRepresentation<someGroup, Coefficient> induceRepresentation(
    GroupRepresentation<someGroup, Coefficient>& in
  );
  template <typename Coefficient>
  GroupRepresentation<someGroup, Coefficient>
  induceRepresentationNormalSubgroup(
    GroupRepresentation<someGroup, Coefficient>& in
  );
...
template <class someGroup, class elementSomeGroup>
bool SubgroupData<someGroup, elementSomeGroup>::
translatableWordsSubgroupElementGetWord(
  ...
  for (int i = 0; i < superword.size; i ++) {
    if (!H.parentRelationship->superGeneratorSubWordExists[superword[i]]) {
      ...
      << "element "
      << g
      << " is assigned parent word "
      << superword.toStringCommaDelimited()
      << " containing generator not found in subgroup "
      << superword[i]
      <<
    ...
    out.addListOnTop(
      H.parentRelationship->superGeneratorSubWords[superword[i]]
    );
  }
  return true;
}
...
template <typename someGroup, typename elementSomeGroup>
bool SubgroupData<someGroup, elementSomeGroup>::sameCosetAs(
  elementSomeGroup& g1, elementSomeGroup& g2
) {
...
int SubgroupData<someGroup, elementSomeGroup>::getCosetId(
  ...
  << gi
  << ") not found in cosets. "
  << "Also some groups are unprintable. "
  << global.fatal;
  return - 1;
}
...
quotientGroupPermutationRepresentation(
  GroupRepresentation<someGroup, Coefficient>& out
) {
  this->computeCosets();
  ...
  global.comments
  << "Subgroup::quotientGroupPermutationRepresentation: Permuting "
  << cosets.size
  << " cosets.\n";
  ...
  for (int i = 0; i < this->groupContent->generators.size; i ++) {
    ...
    for (int ci = 0; ci < out.generators[i].numberOfColumns; ci ++) {
      elementSomeGroup g =
      this->groupContent->generators[i] * this->cosets[ci].representative;
    ...
    << " of coset "
    << this->getCosetId(this->groupContent->generators[i]);
    global.comments
    << " permutes the other cosets as\n"
...
template <typename someGroup, typename elementSomeGroup>
template <typename Coefficient>
GroupRepresentation<someGroup, Coefficient> SubgroupData<
  someGroup, elementSomeGroup
>::induceRepresentationNormalSubgroup(
  GroupRepresentation<someGroup, Coefficient>& in
) {
  ...
  // in TODO: make random FiniteGroups capable of finding their conjugacy
  // classes in less than at least 5 minutes
  ...
  for (int i = 0; i < this->groupContent->generators.size; i ++) {
    ...
    global.comments
    << " represented by "
    << cosets[csi].representative
    << " and corresponds to subgroup element "
    << g;
    in.getMatrixOfElement(g, sr.generators[i]);
    global.comments
    << " which is assigned matrix\n"
...
GroupRepresentation<someGroup, Coefficient> SubgroupData<
  someGroup, elementSomeGroup
>::induceRepresentation(GroupRepresentation<someGroup, Coefficient>& in) {
  GroupRepresentation<someGroup, Coefficient> out;
  out.generators.setSize(this->groupContent->generators.size);
  for (int i = 0; i < this->groupContent->generators.size; i ++) {
    this->computeCosets();
    ...
    if (in.generators.size == 0) {
      ...
      out.generators[i].makeZeroMatrix(
        in.generators[0].numberOfRows * this->cosets.size
      );
    }
    for (int ci = 0; ci < cosets.size; ci ++) {
      ...
      out.generators[i].assignBlock(
        ikblock, kcsi * ikblock.numberOfRows, ci * ikblock.numberOfColumns
      );
    }
  }
  out.ownerGroup = this->groupContent;
  ...
  if (!out.verifyRepresentation()) {
    if (!in.verifyRepresentationExpensive()) {
      global.comments
      << "Well, we weren't given a proper representation either.";
    }
    FiniteGroup<Matrix<Coefficient> > ingroup;
    ingroup.generators = in.generators;
    global.comments
    << "Generator commutation relations for input representation:\n"
    ...
    global.comments
    << "Generator commutation relations for 'representation':\n"
    << outgroup.prettyPrintGeneratorCommutationRelations();
    global.comments
    << "It was supposed to be a quotient group of\n"
...
public:
  SubgroupData<FiniteGroup<ElementWeylGroup>, ElementWeylGroup>
  subGroupDataContainer;
...
public:
  ...
  template <class Coefficient>
  void actByNonSimpleElement(
    int index, const Vectors<Coefficient>& input, Vectors<Coefficient>& output
  ) const {
...
template <typename Coefficient>
class ElementWeylGroupRing: public ElementMonomialAlgebra<
  ElementWeylGroup, Coefficient
> {
public:
...
// characters----------------------------
template <class someFiniteGroup, typename Coefficient>
Coefficient ClassFunction<someFiniteGroup, Coefficient>::innerProduct(
...
template <class someFiniteGroup, typename Coefficient>
ClassFunction<someFiniteGroup, Coefficient> ClassFunction<
  someFiniteGroup, Coefficient
>::operator*(const ClassFunction<someFiniteGroup, Coefficient>& other) const {
...
template <class someFiniteGroup, typename Coefficient>
void ClassFunction<someFiniteGroup, Coefficient>::operator*=(
  const ClassFunction<someFiniteGroup, Coefficient>& right
) {
  if (this->G != right.G) {
    global.fatal
    << "Attempt to multiply class functions belonging to different groups."
    << global.fatal;
  }
...
>::symmetricSecondPower() const {
  ...
  for (int i = 0; i < G->conjugacyClassCount(); i ++) {
    l.data.addOnTop((
        ...
        this->data[G->squaresFirstConjugacyClassRep[i]]
      ) /
      2
    );
  }
...
>::alternateSecondPower() const {
  ...
  for (int i = 0; i < G->conjugacyClassCount(); i ++) {
    l.data.addOnTop((
        this->data[i] *
        this->data[i] - this->data[G->squaresFirstConjugacyClassRep[i]]
      ) /
      2
    );
  }
...
ClassFunction<someFiniteGroup, Coefficient> ClassFunction<
  someFiniteGroup, Coefficient
>::operator-(const ClassFunction& other) const {
  ClassFunction<someFiniteGroup, Coefficient> l = *this;
  l.data += other.data;
  return l;
}
...
public:
  ...
  PolynomialUnivariateDense(
    const PolynomialUnivariateDense<Coefficient>& other
  ) {
  ...
  PolynomialUnivariateDense<Coefficient> operator*(
    const PolynomialUnivariateDense<Coefficient>& right
  ) const;
  ...
  DivisionResult<PolynomialUnivariateDense<Coefficient> > divideBy(
    const PolynomialUnivariateDense<Coefficient>& right
  ) const;
  PolynomialUnivariateDense<Coefficient> operator/(
    const PolynomialUnivariateDense<Coefficient>& divisor
  ) const;
  PolynomialUnivariateDense<Coefficient> operator%(
    const PolynomialUnivariateDense<Coefficient>& divisor
  ) const;
  ...
  // method due to Urbain Le Verrier
};

template <typename Coefficient>
void PolynomialUnivariateDense<Coefficient>::assignMinimalPolynomial(
...
template <typename Coefficient>
void PolynomialUnivariateDense<Coefficient>::assignCharacteristicPolynomial(
  const Matrix<Coefficient>& input
) {
  int n = input.numberOfColumns;
...
template <typename Coefficient>
Coefficient PolynomialUnivariateDense<Coefficient>::operator()(
  const Coefficient& x
) const {
  Coefficient acc = 0;
  Coefficient y = 1;
...
template <typename Coefficient>
void PolynomialUnivariateDense<Coefficient>::operator+=(
  const PolynomialUnivariateDense<Coefficient>& right
) {
...
template <typename Coefficient>
PolynomialUnivariateDense<Coefficient> PolynomialUnivariateDense<Coefficient>::
operator*(const PolynomialUnivariateDense<Coefficient>& right) const {
  PolynomialUnivariateDense<Coefficient> out;
...
template <typename elementSomeGroup>
int FiniteGroup<elementSomeGroup>::multiplyElements(
  int indexLeft, int indexRight
) const {
  return
  this->elements.getIndex(
    this->elements[indexLeft] * this->elements[indexRight]
  );
}
...
bool FiniteGroup<elementSomeGroup>::registerConjugacyClass(
  ...
  getCharacteristicPolynomialStandardRepresentation(characterPolynomial);
  if (
    this->characterPolynomialsConjugacyClassesStandardRepresentation.contains(
      characterPolynomial
    )
  ) {
    for (int i = 0; i < this->conjugacyClasses.size; i ++) {
      elementSomeGroup& otherRepresentative =
      this->conjugacyClasses[i].representative;
      if (!dontAddIfSameInvariants) {
        if (
          this->areConjugate(
            conjugacyClass.representative, otherRepresentative
          )
        ) {
          return false;
        }
        ...
        if (
          !conjugacyClass.representative.hasDifferentConjugacyInvariantsFrom(
            otherRepresentative
          )
        ) {
          return false;
  ...
  this->characterPolynomialsConjugacyClassesStandardRepresentation.addOnTop(
    characterPolynomial
  );
...
template <typename elementSomeGroup>
bool FiniteGroup<elementSomeGroup>::getWord(
  const elementSomeGroup& g, List<int>& word
) {
