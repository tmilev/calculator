...
int do_ex_data_init(openssl_ctx_st *ctx);
...
typedef struct openssl_ctx_method {
    void *(*new_func)(std::stringstream* commentsOnError);
    void (*free_func)(void *);
} OPENSSL_CTX_METHOD;
...
OSSL_EX_DATA_GLOBAL *openssl_ctx_get_ex_data_global(openssl_ctx_st *ctx);
typedef int (openssl_ctx_run_once_fn)(openssl_ctx_st *ctx);
typedef void (openssl_ctx_onfree_fn)(openssl_ctx_st *ctx);

int openssl_ctx_run_once(openssl_ctx_st *ctx, unsigned int idx,
                         openssl_ctx_run_once_fn run_once_fn);
int openssl_ctx_onfree(openssl_ctx_st *ctx, openssl_ctx_onfree_fn onfreefn);

openssl_ctx_st *crypto_ex_data_get_openssl_ctx(const CRYPTO_EX_DATA *ad);
int crypto_new_ex_data_ex(openssl_ctx_st *ctx, int class_index, void *obj,
                          CRYPTO_EX_DATA *ad);
int crypto_get_ex_new_index_ex(openssl_ctx_st *ctx, int class_index,
                               long argl, void *argp,
                               CRYPTO_EX_new *new_func,
                               CRYPTO_EX_dup *dup_func,
                               CRYPTO_EX_free *free_func);
int crypto_free_ex_index_ex(openssl_ctx_st *ctx, int class_index, int idx);
