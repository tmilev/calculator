...
void SemisimpleLieAlgebra::generateLieSubalgebra(
  ...
  for (int i = 0; i < inputOutputGenerators.size; i ++) {
    ...
    if (
      bracket.getRankElementSpan(inputLinearlyIndependent, &seedMons) <
  ...
  for (int i = 0; i < inputOutputGenerators.size; i ++) {
    for (int j = i + 1; j < inputOutputGenerators.size; j ++) {
      ...
      if (
        bracket.getRankElementSpan(inputOutputGenerators, &seedMons) <
        inputOutputGenerators.size
      ) {
...
() {
  ...
  DynkinType
  currentComplementSummand,
  centralizerOfComplementOfCurrentSummand,
  currentSummand;
  HashedList<Rational>
      ...
      if (i != j) {
        ...
        if (
          !nonEmbeddedMe.getMaxQForWhichBetaMinusQAlphaisARoot(
            posRoot1, - posRoot2, alphaStringLength
          )
        ) {
          global.fatal
...
void SlTwoSubalgebra::computeModuleDecompositionsition(
  ...
  for (int j = bufferHighestWeights.size - 1; j >= indexZeroWeight; j --) {
    ...
    if (topMult < 0) {
      global.fatal
      <<
      "The sl(2)-module decomposition shows an sl(2)-module with highest weight "
      << topMult
...
std::string CandidateSemisimpleSubalgebra::toStringDrawWeights(
  ...
  out
  <<
  "<br>Weight diagram. The coordinates corresponding to the simple roots of the "
...
std::string CandidateSemisimpleSubalgebra::toStringModuleDecomposition(
  ...
  out
  <<
  "<tr><td>Module elements (weight vectors). <span class='fElement'>In blue - corresp. F element</span>. "
  ...
  out
  <<
  "<td>Weights of elements in (fundamental coords w.r.t. Cartan of subalgebra) + Cartan centralizer</td>"
  ;
  ...
  out
  <<
  "<td>Single module character over Cartan of s.a.+ Cartan of centralizer of s.a.</td>"
  ;
...
std::string NilradicalCandidate::toString(FormatExpressions* format) const {
  ...
  if (this->flagNilradicalConesIntersect) {
    ...
    if (
      ...
      out
      <<
      "<br>In addition, strong nilradical cone intersections did not suffice, I needed to compute relatively strong intersections. "
      ;
      out
      ...
      out
      <<
      "<br>The highest weight cone consisting of elements commuting with the nilradical subset: "
      << this->toStringTableElementWithWeights(
        this->nonFernandoKacHighestWeightVectorsStrongRelativeToSubset,
        this->nonFernandoKacHighestWeightVectorsStrongRelativeToSubsetWeights
      );
    ...
    if (!this->flagParabolicACExtendsToParabolicAC) {
      out
      <<
      "<br><b style = 'color:red'>Strong centralizer condition not proven to hold.</b>"
      ;
    }
...
std::string CandidateSemisimpleSubalgebra::toStringNilradicalsSummary(
  ...
  if (this->numberOfBadParabolics > 0) {
    out
    <<
    "<br><span style =\"color:#FF0000\">Of the subalgebra(s) satisfying the centralizer condition,  "
    << this->numberOfBadParabolics
...
std::string CandidateSemisimpleSubalgebra::toStringNilradicals(
  ...
  if (this->fernandoKacNilradicalCandidates.size == 0) {
    if (this->owner->flagComputeNilradicals) {
      if (this->isRegularSubalgebra()) {
        return
        "Nilradicals not computed, but that is OK because this a root subalgebra. "
        ;
      } else {
        return
  ...
  if (this->owner->flagProduceLaTeXTables) {
    ...
    for (int i = 0; i < this->fernandoKacNilradicalCandidates.size; i ++) {
      ...
      // { out << " $" <<
      // currentNilradical.nonFernandoKacHighestWeights[j].toStringLetterFormat("\\omega",
      // &tempFormat) << "$";
...
std::string CandidateSemisimpleSubalgebra::toStringCentralizer(
  ...
  if (this->flagCentralizerIsWellChosen && this->centralizerRank != 0) {
    ...
    if (this->indexSemisimplePartCentralizer != - 1) {
      ...
      out
      <<
      " (can't determine subalgebra number - subalgebras computed partially?)";
  ...
  if (
    ...
    out
    <<
    "<br>The semisimple part of the centralizer of the semisimple part of my centralizer: "
    << this->owner->toStringAlgebraLink(
...
std::string CandidateSemisimpleSubalgebra::toStringSystem(
  ...
  if (this->flagSystemSolved) {
    ...
    out
    <<
    "<br><b>Subalgebra candidate not realized, and this may or may not be possible.</b>"
    ;
  }
  out
  << "<br>"
  ...
  if (!this->flagSystemGroebnerBasisFound) {
    out
    <<
    "<br><b>Failed to find Groebner basis of the above system (the computation is too large).</b>"
    ;
  }
...
std::string CandidateSemisimpleSubalgebra::toStringGenerators(
  ...
  out
  <<
  "<br>Dimension of subalgebra generated by predefined or computed generators: "
  << this->basis.size
  << "."
...
std::string CandidateSemisimpleSubalgebra::toString(
  ...
  if (this->isRegularSubalgebra()) {
    out
    <<
    "<br>The subalgebra is regular (= the semisimple part of a root subalgebra). "
    ;
  }
  ...
  if (this->flagSystemSolved && !shortReportOnly) {
    ...
    if (
      ...
      out
      <<
      "As the centralizer is well-chosen and the centralizer of our subalgebra is "
    ...
    out
    <<
    "<table class='tableStandard'><tr><td>Highest vectors of representations (total "
    << this->highestVectorsNonSorted.size
    << ")";
    if (this->flagCentralizerIsWellChosen) {
    ...
    if (
      ...
      out
      <<
      "<tr><td>weights rel. to Cartan of (centralizer+semisimple s.a.). </td>";
...
void CandidateSemisimpleSubalgebra::getHsScaledToActByTwoByType(
  ...
  if (
    ...
    global.fatal
    <<
    "AllTypes.size must equal this->cartanSubalgebrasByComponentScaledToActByTwo.size. "
    << global.fatal;
  }
