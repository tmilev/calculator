...
static void init_write_state_machine(struct ssl_connection_st* s);
static SUB_STATE_RETURN write_state_machine(struct ssl_connection_st* s);

OSSL_HANDSHAKE_STATE SSL_get_state(const SSL* ssl) {
  const struct ssl_connection_st* sc = SSL_CONNECTION_FROM_CONST_SSL(ssl);
  if (sc == NULL) return TLS_ST_BEFORE;
...
OSSL_HANDSHAKE_STATE ossl_statem_get_state(struct ssl_connection_st* s) {
 ...
 */
void ossl_statem_clear(struct ssl_connection_st* s) {
 ...
 */
void ossl_statem_set_renegotiate(struct ssl_connection_st* s) {
  ossl_statem_set_in_init(s, 1);
  s->statem.request_state = TLS_ST_SW_HELLO_REQ;
}

void ossl_statem_send_fatal(struct ssl_connection_st* s, int al) {
  /* We shouldn't call SSLfatal() twice. Once is enough */
  if (s->statem.in_init && s->statem.state == MSG_FLOW_ERROR) return;
  ossl_statem_set_in_init(s, 1);
  s->statem.state = MSG_FLOW_ERROR;
  if (al != SSL_AD_NO_ALERT) ssl3_send_alert(s, SSL3_AL_FATAL, al);
}
 ...
 */
void ossl_statem_fatal(
  struct ssl_connection_st* s, int al, int reason, const char* fmt, ...
) {
  va_list args;
  va_start(args, fmt);
...
void ossl_statem_set_in_init(struct ssl_connection_st* s, int init) {
  ...
  if (
    s->rlayer.rrlmethod != NULL && s->rlayer.rrlmethod->set_in_init != NULL
  ) {
...
static info_cb get_callback(struct ssl_connection_st* s) {
...
int state_machine_wrap_up(
  struct ssl_connection_st* s, int server, int result
) {
...
int initialize_state_machine(
  ...
  if ((SSL_in_before(ssl)) || s->renegotiate) {
    if (!tls_setup_handshake(s)) {
      /* SSLfatal() already called */ return - 1;
    }
...
static SUB_STATE_RETURN read_state_machine(struct ssl_connection_st* s) {
  ...
  int ret, mt;
  size_t len = 0;
  int(*transition)(struct ssl_connection_st* s, int mt);
  PACKET pkt;
  MSG_PROCESS_RETURN(*process_message)(
    struct ssl_connection_st* s, PACKET* pkt
  );
  WORK_STATE(*post_process_message)(
    struct ssl_connection_st* s, WORK_STATE wst
  );
  size_t(*max_message_size)(struct ssl_connection_st* s);
  void(*cb)(const SSL* ssl, int type, int val) = NULL;
  SSL* ssl = SSL_CONNECTION_GET_SSL(s);
  cb = get_callback(s);
  if (s->server) {
  ...
  while (1) {
    ...
    case READ_STATE_HEADER:
      ...
      if (ret == 0) {
        /* Could be non-blocking IO */ return SUB_STATE_ERROR;
      }
      if (cb != NULL) {
 ...
 */
static int statem_do_write(struct ssl_connection_st* s) {
  OSSL_STATEM* st = &s->statem;
  if (
    st->hand_state == TLS_ST_CW_CHANGE || st->hand_state == TLS_ST_SW_CHANGE
  ) {
    if (SSL_CONNECTION_IS_DTLS(s)) return
    dtls1_do_write(s, SSL3_RT_CHANGE_CIPHER_SPEC); else return
    ssl3_do_write(s, SSL3_RT_CHANGE_CIPHER_SPEC);
  } else {
...
static SUB_STATE_RETURN write_state_machine(struct ssl_connection_st* s) {
  OSSL_STATEM* st = &s->statem;
  int ret;
  WRITE_TRAN(*transition)(struct ssl_connection_st* s);
  WORK_STATE(*pre_work)(struct ssl_connection_st* s, WORK_STATE wst);
  WORK_STATE(*post_work)(struct ssl_connection_st* s, WORK_STATE wst);
  int(*get_construct_message_f)(
    struct ssl_connection_st* s,
    CON_FUNC_RETURN(**confunc)(struct ssl_connection_st* s, WPACKET* pkt),
    int* mt
  );
  void(*cb)(const SSL* ssl, int type, int val) = NULL;
  CON_FUNC_RETURN(*confunc)(struct ssl_connection_st* s, WPACKET* pkt);
  ...
  while (1) {
    ...
    case WRITE_STATE_TRANSITION:
      if (cb != NULL) {
        /* Notify callback of an impending state change */
        if (s->server) cb(ssl, SSL_CB_ACCEPT_LOOP, 1); else cb(
          ssl, SSL_CB_CONNECT_LOOP, 1
        );
      }
      switch (transition(s)) {
    ...
    case WRITE_STATE_PRE_WORK:
      ...
      if (!get_construct_message_f(s, &confunc, &mt)) {
        /* SSLfatal() already called */ return SUB_STATE_ERROR;
      }
      if (mt == SSL3_MT_DUMMY) {
        /* Skip construction and sending. This isn't a "real" state */ st->
        write_state =
        WRITE_STATE_POST_WORK;
        st->write_state_work = WORK_MORE_A;
        break;
      ...
      if (
        !ssl_close_construct_packet(s, &pkt, mt) || !WPACKET_finish(&pkt)
      ) {
        WPACKET_cleanup(&pkt);
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return SUB_STATE_ERROR;
 ...
 * Flush the write BIO
 */
int statem_flush(struct ssl_connection_st* s) {
 ...
 */
int ossl_statem_app_data_allowed(struct ssl_connection_st* s) {
  OSSL_STATEM* st = &s->statem;
  if (st->state == MSG_FLOW_UNINITED) return 0;
  if (!s->s3.in_read_app_data || (s->s3.total_renegotiations == 0)) return 0;
  if (s->server) {
    /*
         ...
         */
    if (
      st->hand_state == TLS_ST_BEFORE || st->hand_state == TLS_ST_SR_CLNT_HELLO
    ) return 1;
  } else {
    /*
         ...
         * allow app data
         */
    if (st->hand_state == TLS_ST_CW_CLNT_HELLO) return 1;
  }
  return 0;
}
 ...
 */
int ossl_statem_export_allowed(struct ssl_connection_st* s) {
  return
 ...
 * or 0 if otherwise.
 */
int ossl_statem_export_early_allowed(struct ssl_connection_st* s) {
  /*
