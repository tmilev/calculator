...
std::string SyntacticElement::getIntegerStringCrashIfNot(
  ...
  if (this->controlIndex != owner.conInteger()) {
    global.fatal
    << "Request to get rational from a non-rational element. "
    << global.fatal;
  }
...
std::string SyntacticElement::toStringDebug(const CalculatorParser& owner)
const {
  std::stringstream out;
  out << this->toStringSyntaxRole(owner) << "[" << this->source << "]";
  return out.str();
}
...
std::string SyntacticElement::toStringSyntaxRole(const CalculatorParser& owner)
const {
...
bool CalculatorParser::parse(
  ...
  if (
    !this->parseAndExtractExpressions(
      input, output, this->toBeParsed, this->syntacticStack, nullptr
    )
  ) {
    return false;
  }
...
void CalculatorParser::initializeControlSequences() {
  ...
  this->controlSequences.addOnTopNoRepetitionMustBeNew("Letters");
  this->controlSequences.addOnTopNoRepetitionMustBeNew("Digits");
  this->controlSequences.addOnTopNoRepetitionMustBeNew(
    "QuoteUnderConstruction"
  );
  this->controlSequences.addListOnTop(this->owner->operations.keys);
  // all operations defined up to this point are also control sequences
...
bool CalculatorParser::parseAndExtractExpressionsDefault(
  ...
  return
  this->parseAndExtractExpressions(
    input, output, this->toBeParsed, this->syntacticStack, &this->syntaxErrors
  );
}
...
bool CalculatorParser::parseAndExtractExpressions(
  ...
  this->currrentlyParsed = &outputAllSyntacticElements;
...
std::string CalculatorParser::toStringSyntacticStackHTMLSimple() {
  ...
  *(*this->currentSyntacticStack).lastObject();
  ...
  for (
    ...
    if (currentElement.controlIndex != this->conExpression()) {
      out
      << " <b style='color:red'>"
      << currentElement.toStringSyntaxRole(*this)
      << "</b> ";
    } else {
...
bool CalculatorParser::replaceOXEXEXEXByE() {
  ...
  SyntacticElement& middleE = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    4
  ];
...
bool CalculatorParser::replaceSqrtEXByEX() {
  ...
  SyntacticElement& argument = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
...
bool CalculatorParser::replaceSqrtEXXByEXX() {
  ...
  (*this->currentSyntacticStack)[lastSize - 2] = (
    *this->currentSyntacticStack
  )[lastSize - 1];
...
bool CalculatorParser::decreaseStackExceptLast(int decrease) {
  ...
  (*this->currentSyntacticStack)[(*this->currentSyntacticStack).size -
    decrease -
    1
  ] =
  *this->currentSyntacticStack->lastObject();
...
bool CalculatorParser::replaceOXXByEXX() {
  ...
  left.data.makeAtom(
    *this->owner, this->getOperationIndexFromControlIndex(left.controlIndex)
  );
  left.controlIndex = this->conExpression();
...
bool CalculatorParser::replaceXXXXXByCon(int controlIndex) {
  (*this->currentSyntacticStack)[(*this->currentSyntacticStack).size - 5].
  controlIndex =
  controlIndex;
...
bool CalculatorParser::replaceXXXXXByConCon(
...
bool CalculatorParser::replaceXByO(int operation) {
  ...
  controlIndex =
  this->conExpression();
  (*this->currentSyntacticStack)[(*this->currentSyntacticStack).size - 1].data.
  makeAtom(*this->owner, operation);
  return true;
}
...
void CalculatorParser::parseFillDictionaryDefaultOutputDeprecated(
  const std::string& input
) {
  STACK_TRACE("CalculatorParser::parseFillDictionaryDefaultOutputDeprecated");
  this->parseFillDictionaryDeprecated(input, *this->currrentlyParsed);
...
SyntacticElement CalculatorParser::makeFromSingleCharacter(char input) {
  ...
  this->controlSequences.getIndex(result.source);
  if (indexInControlSequences >= 0) {
    result.controlIndex = indexInControlSequences;
  }
...
void CalculatorParser::parseFillDictionaryDeprecated(
  const std::string& input, List<SyntacticElement>& output
) {
  STACK_TRACE("CalculatorParser::parseFillDictionaryDeprecated");
  std::string current;
...
int CalculatorParser::getOperationIndexFromControlIndex(int controlIndex) {
  return
  this->owner->operations.getIndex(this->controlSequences[controlIndex]);
}
...
replaceEXXSequenceXBy_Expression_with_E_instead_of_sequence() {
  SyntacticElement& sequenceElement = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
  SyntacticElement& functionExpressionlt = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    5
  ];
...
bool CalculatorParser::replaceXEXByEContainingOE(int inputOpIndex) {
  SyntacticElement& outputElement = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    3
  ];
  SyntacticElement& inputElement = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
  ...
  outputElement.controlIndex = this->conExpression();
  return this->decreaseStackSetCharacterRanges(2);
}
...
bool CalculatorParser::replaceOOEEXbyEXpowerLike() {
  ...
  if (this->flagLogSyntaxRules) {
    this->lastRuleName = "[Calculator::replaceOOEEXbyEXpowerLike]";
  }
...
bool CalculatorParser::replaceMatrixXByE() {
  SyntacticElement& matrixElement = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
...
bool CalculatorParser::allowsApplyFunctionInPreceding(
  ...
  return role != SyntacticElement::LEFT_CURLY_BRACE&&
...
bool CalculatorParser::replaceSequenceUXEYBySequenceZY(int controlIndex) {
  ...
  SyntacticElement& afterleft = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    3
  ];
...
bool CalculatorParser::replaceEOXbyEX() {
  ...
  SyntacticElement& operationElement = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
...
bool CalculatorParser::replaceSsSsXdotsXbySsXdotsX(int numberOfDots) {
  ...
  left.controlIndex = this->conSequenceStatements();
  (*this->currentSyntacticStack).popIndexShiftDown((
      *this->currentSyntacticStack
    ).size -
    numberOfDots -
    1
...
bool CalculatorParser::replaceEPowerMinusEXByEX() {
  ...
  SyntacticElement& minusExponent = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
...
bool CalculatorParser::allowsTensorInPreceding(const SyntacticElement& lookAhead) {
  return this->allowsTimesInPreceding(lookAhead) || lookAhead.syntacticRole == SyntacticElement::TENSOR_PRODUCT;
}

bool CalculatorParser::allowsTimesInNext(const SyntacticElement& preceding) {
  if(
          preceding.syntacticRole == SyntacticElement::LEFT_CURLY_BRACE ||
      preceding.syntacticRole == SyntacticElement::UPPER_CARET  ||
      preceding.syntacticRole == SyntacticElement::UNDERSCORE ||
      preceding.source== "\\sqrt"
...
bool CalculatorParser::allowsTimesInPreceding(const SyntacticElement& lookAhead) {
...
bool CalculatorParser::allowsDivideInPreceding(const SyntacticElement& lookAhead) {
  return this->allowsTimesInPreceding(lookAhead);
}
...
bool CalculatorParser::extractExpressionsFromPreprocessed(
  ...
  (*this->currentSyntacticStack).reserve((*this->currrentlyParsed).size +
  ...
  for (
    ...
    if (counterReport >= symbolsToIssueReport) {
      ...
      if (
        ...
        << " out of "
        << (*this->currrentlyParsed).size
        << " syntactic elements. ";
        report.report(reportStream.str());
      }
    }
    (*this->currentSyntacticStack).addOnTop((*this->currrentlyParsed)[
        this->counterInToBeParsed
      ]
    );
    if (this->flagLogSyntaxRules) {
...
bool CalculatorParser::applyOneRule() {
  ...
  SyntacticElement& lastExpression = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    1
  ];
  ...
  SyntacticElement& secondToLastExpression = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    2
  ];
  const std::string& secondToLastS =
  this->controlSequences[secondToLastExpression.controlIndex];
  ...
  if (
    lastExpression.syntacticRole == SyntacticElement::RAW &&
    this->controlSequences.contains(lastExpression.source)
  ) {
    lastExpression.controlIndex =
    this->controlSequences.getIndex(lastExpression.source);
    lastExpression.syntacticRole = SyntacticElement::UNKNOWN;
  ...
  SyntacticElement& thirdToLastExpression = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    3
  ];
  ...
  if (
    ...
    secondToLastExpression.source += lastExpression.source;
    secondToLastExpression.controlIndex = lastExpression.controlIndex;
  ...
  if (
    ...
    int controlIndex =
    this->controlSequences.getIndex(secondToLastExpression.source);
    if (keywordIndex < 0 || controlIndex < 0) {
  ...
  this->controlSequences[thirdToLastExpression.controlIndex];
  SyntacticElement& fourthToLastExpression = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    4
  ];
  ...
  const std::string& fifthToLastS =
  this->controlSequences[fifthToLastE.controlIndex];
  const SyntacticElement& sixthToLastExpression = (
    *this->currentSyntacticStack
  )[(*this->currentSyntacticStack).size - 6];
  SyntacticElement::Role sixthToLastRole = sixthToLastExpression.syntacticRole;
  const std::string& sixthToLastS =
  this->controlSequences[sixthToLastExpression.controlIndex];
  const SyntacticElement& seventhToLastE = (*this->currentSyntacticStack)[(
      *this->currentSyntacticStack
    ).size -
    7
  ];
  ...
  SyntacticElement::Role seventhToLastRole = seventhToLastE.syntacticRole;
  ...
  if (secondToLastRole == SyntacticElement::BACKSLASH_INT && lastRole == SyntacticElement::EXPRESSION) {
    return this->replaceOXbyEX();
  }
  if (thirdToLastRole == SyntacticElement::BACKSLASH_INT && secondToLastRole == SyntacticElement::EXPRESSION) {
    return this->replaceOXXByEXX();
  }
  if (secondToLastRole == SyntacticElement::INTEGRAL_WITH_SUB_AND_SUPERSCRIPT && lastRole == SyntacticElement::EXPRESSION) {
    return this->replaceAXbyEX();
  }
  if (thirdToLastRole ==  SyntacticElement::INTEGRAL_WITH_SUB_AND_SUPERSCRIPT && secondToLastRole == SyntacticElement::EXPRESSION) {
  ...
  if ((fifthToLastRole == SyntacticElement::BACKSLASH_INT || this->isDefiniteIntegral(fifthToLastRole)) &&
    fourthToLastRole == SyntacticElement::EXPRESSION  && (
          thirdToLastRole ==SyntacticElement::PLUS || thirdToLastRole == SyntacticElement::MINUS
    ) &&
    secondToLastRole == SyntacticElement::EXPRESSION  &&
    this->canBeRegardedAsDifferentialForm(lastExpression)
  ) {
...
&&
    ...
    fourthToLastExpression.syntacticRole = SyntacticElement::INTEGRAL_WITH_SUBSCRIPT;
    fourthToLastExpression.controlIndex=this->controlSequences.getIndexNoFail("\\int_{*}");
    return this->decreaseStackExceptLast(2);
  }
  if (
      fourthToLastRole ==SyntacticElement::BACKSLASH_INT &&
    ...
    fourthToLastExpression.syntacticRole =SyntacticElement::INTEGRAL_WITH_SUPERSCRIPT ;
        fourthToLastExpression.controlIndex = this->controlSequences.getIndexNoFail("\\int^{*}");
    return this->decreaseStackExceptLast(2);
  }
  if (
    ...
    thirdToLastExpression.syntacticRole = SyntacticElement::INTEGRAL_WITH_SUB_AND_SUPERSCRIPT;
    thirdToLastExpression.controlIndex=this->controlSequences.getIndexNoFail("\\int_{*}^{**}");
  ...
  if (
    ...
    fourthToLastExpression.controlIndex = this->controlSequences.getIndexNoFail( "\\int_{*}^{**}");
    fourthToLastExpression.syntacticRole = SyntacticElement::INTEGRAL_WITH_SUB_AND_SUPERSCRIPT;
  ...
  if (
   ...
   fourthToLastExpression.controlIndex =
        this->controlSequences.getIndexNoFail("\\int_{*}^{**}");
...
secondToLastExpression.syntacticRole =SyntacticElement::EXPRESSION;
  ...
  if (thirdToLastS == "\\text" && secondToLastRole == SyntacticElement::EXPRESSION) {
  ...
  // if (fourthToLastS == "Expression" && thirdToLastS == "{}" && secondToLastS
  // == "Expression"
  ...
  if (secondToLastRole == SyntacticElement::EXPRESSION && lastS == "!") {
  ...
  if (
    ...
    if (this->flagLogSyntaxRules) {
      this->lastRuleName = "(expression expression any) to (expression times expression any)";
    }
    thirdToLastExpression.data.makeXOX(*this->owner, this->owner->opTimes(), thirdToLastExpression.data, secondToLastExpression.data);


    Expression impliedFunApplication;
    if (
      CalculatorBasics::timesToFunctionApplication(
        *this->owner, thirdToLastExpression.data,
        impliedFunApplication
      )
    ) {
     thirdToLastExpression.
      data =
      impliedFunApplication;
      if (this->flagLogSyntaxRules) {
        this->lastRuleName = "(expression expression any) to (expression of expression any)";
      }
    }
    return    this->decreaseStackExceptLast(1);
  ...
  if (thirdToLastS == "|" && secondToLastRole == SyntacticElement::EXPRESSION && lastS == "|") {
    ...
    if (this->owner->flagUseBracketsForIntervals) {
      if (secondToLastExpression.data.isSequenceNElements(2)) {
        if (
          ...
          return
          this->replaceXEXByEContainingOE(this->owner->opIntervalOpen());
        }
      }
    }
  }
  ...
  if (
    ...
    return
    this->replaceXByCon(
      this->controlSequences.getIndexNoFail("MakeSequence")
    );
  }
  if (thirdToLastS == "\\sqrt" && secondToLastRole == SyntacticElement::EXPRESSION) {
    return this->replaceSqrtEXByEX();
  }
  if (fourthToLastS == "\\sqrt" && thirdToLastRole == SyntacticElement::EXPRESSION) {
  ...
  if (
    thirdToLastS == "if" &&
    secondToLastRole==SyntacticElement::EXPRESSION  &&
    this->allowsIfInPreceding(lastS)
  ) {
    return this->replaceOXXByEXX();
  }
  if (
    fourthToLastS == "Sequence" &&
    thirdToLastS == "," &&
    secondToLastRole==SyntacticElement::EXPRESSION  && (
      lastS == "," ||
      lastS == ")" ||
  ...
  if (
    secondToLastS == "Sequence" && ((thirdToLastS == "(" && lastS == ")") || (
        thirdToLastS == "{" && lastS == "}"
      ) || (
...
&&
  ...
  if (
    ...
    secondToLastRole==SyntacticElement::EXPRESSION && (lastS == ")" || lastS == "}")
  ) {
  ...
  if (secondToLastRole==SyntacticElement::EXPRESSION && lastS == "EndProgram") {
    this->replaceEXdotsXbySsXdotsX(1);
  ...
  if (secondToLastS == "SequenceStatements" && lastRole ==SyntacticElement::END_PROGRAM) {
