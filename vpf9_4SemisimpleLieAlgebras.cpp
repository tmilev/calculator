...
{ MacroRegisterFunctionWithName("SemisimpleSubalgebras::ToString");
  ...
  << " the corresponding Lie algebras by dimension). <br>If ranks and types are equal, the simple components "
  << " are compared by the upper indices. Finally, if the upper indices are equal, the simple "
  << " components are ordered arbitrarily (usually, in the order in which they were generated by the algorithm)."
  << "<br>We now explain the upper index of the Dynkin types. The upper index is, up to a type-dependent (but "
  << "not rank dependent) adjustment, proportional to the Dynkin index. Fix a Dynkin type, say A^3_2 "
  << "(rank 2, type A, upper index 3). "
  << "The upper index 3 stands for the length of the first co-root squared, that is, the length of "
  << " the element of the Cartan corresponding to the first weight space. Here, the element of the "
  << "Cartan is scaled so that it acts by the constant 2 on the corresponding positive weight space. "
  << "<hr> ";
  int counter=0;
  for (int i=0; i<this->Hcandidates.size; i++)
