...
const knownTypes = {
  ...
  horizontalBrace: new MathNodeType({
    'type': 'horizontalBrace',
  }),
...
class MathNodeFactory {
  ...
  error(
    ...
    result.errorContent = initialContent;
    result.initialContent = "Error: " + this.shortenString(initialContent);
    return result;
  }
  ...
  shortenString(
    ...
    const end = input.substring(input.length - 6);
    return `${start} ... ${end}`;
  }
...
function mathFromElement(
  ...
  return mathFromLatex(
    ...
    callback, useSVG, useMathML, copyButton);
}
...
class EquationEditorOptions {
  constructor(
     ...
     * useSVG: {boolean|null|undefined},
     * useMathML: {boolean|null|undefined},
     * }}
     */
    options,
  ) {
    ...
    /** @type {boolean} */
    this.useSVG = false;
    if (options.useSVG === true) {
      this.useSVG = true;
    }
    /** @type {boolean} */
    this.useMathML = false;
    if (options.useMathML === true) {
      this.useMathML = true;
    }
...
class FocusInformation {
  ...
  toHtmlDebugData() {
    ...
    result.push(document.createTextNode(
      `Last focused: ${this.lastFocused.toString()}, position: ${this.lastFocused.positionCursorBeforeKeyEvents}`));
...
class EquationEditor {
  ...
  handleMouseSelectionStart(
    ...
    if (this.options.debugLogContainer !== null) {
      this.lastSelectionAction =
        `Mouse selection start over ${element.toString()}, position:${element.positionCursorBeforeKeyEvents}.`;
    }
...
class MathNodeSqrt extends MathNode {
  ...
  toScalableVectorGraphics(
    ...
    let path = `M ${c.decorationLeft} ${c.decorationY} L ${c.decorationRight} ${c.decorationY} `;
...
class MathNodeHorizontalBrace extends MathNode {
  ...
  computeDimensions() {
    ...
    const middleLeft = this.children[2];
    ...
    if (this.pointsUp) {
      left.element.style.borderTopLeftRadius = radiusString;
      left.element.style.borderTop = borderString;
    ...
    left.boundingBox.width = desiredRadius;
    right.boundingBox.width = desiredRadius;
    middleLeft.boundingBox.width = desiredRadius;
    middleRight.boundingBox.width = desiredRadius;

    left.boundingBox.height = desiredHeight;
    middleLeft.boundingBox.height = desiredHeight;
    middleRight.boundingBox.height = desiredHeight;
    right.boundingBox.height = desiredHeight;
    if (this.pointsUp) {
      left.boundingBox.top = desiredHeight;
    ...
    middleLeft.boundingBox.left = halfWidth - desiredRadius;
    middleRight.boundingBox.left = halfWidth;
    right.boundingBox.left = desiredWidth - desiredRadius;
...
class MathNodeSquareBracketsLike extends MathNodeDelimiterMark {
  constructor(
    /** @type {EquationEditor!} */
    equationEditor,
    /** @type {boolean} */
...
class MathNodeCurlyBrace extends MathNodeDelimiterMark {
  ...
  toScalableVectorGraphics(
    ...
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceBottom} ${c.xMiddle} ${c.yLow - c.radius} `;
    path += `L ${c.xMiddle} ${c.yMiddle + c.radius} `;
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceOther} ${c.xStickyPart} ${c.yMiddle} `;
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceOther} ${c.xMiddle} ${c.yMiddle - c.radius} `;
    path += `L ${c.xMiddle} ${c.yHigh + c.radius} `;
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceBottom} ${c.xEnd} ${c.yHigh} `;

    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceOther} ${c.xMiddle + c.thicknessShift} ${c.yHigh + c.radius} `;
    path += `L ${c.xMiddle + c.thicknessShift} ${c.yMiddle - c.radius} `;
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceBottom} ${c.xStickyPart + c.thicknessShift} ${c.yMiddle} `;
    path += `A ${c.radius} ${c.radius} 0 0 ${c.arcChoiceBottom} ${c.xMiddle + c.thicknessShift} ${c.yMiddle + c.radius} `;
...
class MathTagConverter {
   ...
   * mathML: generate a mathML rendering.
  ...
  constructor(
    ...
    /** @type {boolean} */
    this.mathML = /** @type {boolean} */ (options.mathML);
    if (this.mathML === undefined || this.mathML === null) {
      this.mathML = false;
    }
    /** @type {boolean} */
  ...
  convertTags(
    ...
    for (let i = 0; i < toBeConverted.length; i++) {
      ...
      if (this.svgOnly) {
        mathTag.className = 'mathcalculatorSVG';
        this.mathElementsSVG.push(mathTag);
      } else if (this.mathML) {
        mathTag.className = 'mathcalculatorMathML';
        this.mathElementsSVG.push(mathTag);
      } else {
  ...
  typesetMathTags(
    ...
    if (this.elementsToTypeset < 0) {
      this.elementsToTypeset = this.allMathElements.length;
      this.typesetTotal = 0;
    }
    for (; this.typesetTotal < this.allMathElements.length;
      this.typesetTotal++) {
