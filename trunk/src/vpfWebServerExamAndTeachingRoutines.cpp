...
std::string WebWorker::GetProblemSolution()
{ MacroRegisterFunctionWithName("WebWorker::GetProblemSolution");
  double startTime=theGlobalVariables.GetElapsedSeconds();
  CalculatorHTML theProblem;
  theProblem.LoadCurrentProblemItem();
  std::stringstream out;
...
{ MacroRegisterFunctionWithName("WebWorker::ProcessProblemSolution");
  stOutput << WebWorker::GetProblemSolution();
  return 0;
}
...
//  stOutput << "<b>debug remove when done: Random seed: " << theProblem.theProblemData.randomSeed << "</b>";
  ...
  for (int i=0; i<theArgs.size(); i++)
    ...
    { int newAnswerIndex=theProblem.GetAnswerIndex(studentAnswerNameReader);
      ...
      { stOutput << "<b>You submitted two or more answers [answer tags: "
      ...
      if (answerIdIndex==-1)
      { stOutput << "<b> You submitted an answer to tag with id "
